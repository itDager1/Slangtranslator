import { Hono } from "npm:hono";
import { cors } from "npm:hono/cors";
import { logger } from "npm:hono/logger";
import * as kv from "./kv_store.tsx";

const app = new Hono();

// Enable logger
app.use('*', logger(console.log));

// Enable CORS for all routes and methods
app.use(
  "/*",
  cors({
    origin: "*",
    allowHeaders: ["Content-Type", "Authorization"],
    allowMethods: ["GET", "POST", "PUT", "DELETE", "OPTIONS"],
    exposeHeaders: ["Content-Length"],
    maxAge: 600,
  }),
);

// ========================
// TYPES & DATA
// ========================

interface DictionaryEntry {
  word: string;
  definition: string;
  shortTranslation?: string;
  examples: string[];
  category: string;
  explanations?: Array<{ word: string; explanation: string }>;
}

// Word forms mapping - links various forms of a word to its base form
const wordFormsMap: Record<string, string> = {
  // Базовые формы всех слов из словаря (для быстрого поиска)
  "чилить": "чилить",
  "скучно": "скучно",
  "батхёрт": "батхёрт",
  "криво": "криво",
  "лол": "лол",
  "баян": "баян",
  "рофлить": "рофлить",
  "краш": "краш",
  "флексить": "флексить",
  "флекс": "флекс",
  "вайб": "вайб",
  "кринж": "кринж",
  "токсик": "токсик",
  "хейтить": "хейтить",
  "леймить": "леймить",
  "агриться": "агриться",
  "зумер": "зумер",
  "бумер": "бумер",
  "изи": "изи",
  "го": "го",
  "рандом": "рандом",
  "спойлер": "спойлер",
  "стримить": "стримить",
  "донатить": "донатить",
  "пранк": "пранк",
  "троллить": "троллить",
  "шипперить": "шипперить",
  "сасный": "сасный",
  "мем": "мем",
  "хайп": "хайп",
  "годнота": "годнота",
  "гайд": "гайд",
  "лейм": "лейм",
  "чувак": "чувак",
  "имба": "имба",
  "нуб": "нуб",
  "про": "про",
  "дроп": "дроп",
  "лайк": "лайк",
  "репост": "репост",
  "селфи": "селфи",
  "контент": "контент",
  "тян": "тян",
  "фейк": "фейк",
  "лут": "лут",
  "скилл": "скилл",
  "лаг": "лаг",
  "баг": "баг",
  "читер": "читер",
  "респ": "респ",
  "фраг": "фраг",
  "кемпер": "кемпер",
  "рофл": "рофл",
  "кек": "кек",
  "вау": "вау",
  "ору": "ору",
  "сорян": "сорян",
  "чел": "чел",
  "норм": "норм",
  "кста": "кста",
  "братан": "братан",
  "кореш": "кореш",
  "дрип": "дрип",
  "топ": "топ",
  "огонь": "огонь",
  "зашквар": "зашквар",
  "движ": "движ",
  "рил": "рил",
  "челлендж": "челлендж",
  "влог": "влог",
  "блогер": "блогер",
  "реакция": "реакция",
  "трек": "трек",
  "френдзона": "френдзона",
  "экс": "экс",
  "флиртовать": "флиртовать",
  "симпатия": "симпатия",
  
  // Формы глагола "чилить"
  "чилил": "чилить",
  "чилила": "чилить",
  "чилило": "чилить",
  "чилили": "чилить",
  "чилю": "чилить",
  "чилишь": "чилить",
  "чилит": "чилить",
  "чилим": "чилить",
  "чилите": "чилить",
  "чилят": "чилить",
  "чилящий": "чилить",
  "почилить": "чилить",
  "почилил": "чилить",
  "почилила": "чилить",
  "почилим": "чилить",
  "чиль": "чилить",
  "чильте": "чилить",
  
  // Формы "рофлить"
  "рофлил": "рофлить",
  "рофлила": "рофлить",
  "рофлило": "рофлить",
  "рофлили": "рофлить",
  "рофлю": "рофлить",
  "рофлишь": "рофлить",
  "рофлит": "рофлить",
  "рофлим": "рофлить",
  "рофлите": "рофлить",
  "рофлят": "рофлить",
  "рофли": "рофлить",
  "порофлить": "рофлить",
  "порофлил": "рофлить",
  
  // Формы "флексить"
  "флексил": "флексить",
  "флексила": "флексить",
  "флексило": "флексить",
  "флексили": "флексить",
  "флекшу": "флексить",
  "флексишь": "флексить",
  "флексит": "флексить",
  "флексим": "флексить",
  "флексите": "флексить",
  "флексят": "флексить",
  "флекси": "флексить",
  "пофлексить": "флексить",
  
  // Формы существительного "флекс"
  "флекса": "флекс",
  "флексу": "флекс",
  "флексом": "флекс",
  "флексе": "флекс",
  "флексы": "флекс",
  "флексов": "флекс",
  "флексам": "флекс",
  "флексами": "флекс",
  "флексах": "флекс",
  
  // Формы "хейтить"
  "хейтил": "хейтить",
  "хейтила": "хейтить",
  "хейтило": "хейтить",
  "хейтили": "хейтить",
  "хейчу": "хейтить",
  "хейтишь": "хейтить",
  "хейтит": "хейтить",
  "хейтим": "хейтить",
  "хейтите": "хейтить",
  "хейтят": "хейтить",
  "хейть": "хейтить",
  "похейтить": "хейтить",
  "хейтер": "хейтить",
  "хейтеры": "хейтить",
  "хейтеров": "хейтить",
  
  // Формы "леймить"
  "леймил": "леймить",
  "леймила": "леймить",
  "леймило": "леймить",
  "леймили": "леймить",
  "леймлю": "леймить",
  "леймишь": "леймить",
  "леймит": "леймить",
  "леймим": "леймить",
  "леймите": "леймить",
  "леймят": "леймить",
  "лейми": "леймить",
  "полеймить": "леймить",
  "залеймить": "леймить",
  
  // Формы "агриться"
  "агрился": "агриться",
  "агрилась": "агриться",
  "агрилось": "агриться",
  "агрились": "агриться",
  "агрюсь": "агриться",
  "агришься": "агриться",
  "агрится": "агриться",
  "агримся": "агриться",
  "агритесь": "агриться",
  "агрятся": "агриться",
  "агрись": "агриться",
  "заагриться": "агриться",
  
  // Формы "стримить"
  "стримил": "стримить",
  "стримила": "стримить",
  "стримило": "стримить",
  "стримили": "стримить",
  "стримлю": "стримить",
  "стримишь": "стримить",
  "стримит": "стримить",
  "стримим": "стримить",
  "стримите": "стримить",
  "стримят": "стримить",
  
  // Формы "донатить"
  "донатил": "донатить",
  "донатила": "донатить",
  "донатило": "донатить",
  "донатили": "донатить",
  "доначу": "донатить",
  "донатишь": "донатить",
  "донатит": "донатить",
  "донатим": "донатить",
  "донатите": "донатить",
  "донатят": "донатить",
  "задонатил": "донатить",
  "задонатила": "донатить",
  "задонатили": "донатить",
  
  // Формы "троллить"
  "троллил": "троллить",
  "троллила": "троллить",
  "троллило": "троллить",
  "троллили": "троллить",
  "троллю": "троллить",
  "троллишь": "троллить",
  "троллит": "троллить",
  "троллим": "троллить",
  "троллите": "троллить",
  "троллят": "троллить",
  "тролль": "троллить",
  "потроллить": "троллить",
  "затроллить": "троллить",
  
  // Формы "шипперить"
  "шипперил": "шипперить",
  "шипперила": "шипперить",
  "шипперило": "шипперить",
  "шипперили": "шипперить",
  "шипперю": "шипперить",
  "шипперишь": "шипперить",
  "шипперит": "шипперить",
  "шипперим": "шипперить",
  "шипперите": "шипперить",
  "шипперят": "шипперить",
  
  // Формы "хайпить"
  "хайпил": "хайпить",
  "хайпила": "хайпить",
  "хайпило": "хайпить",
  "хайпили": "хайпить",
  "хайплю": "хайпить",
  "хайпишь": "хайпить",
  "хайпит": "хайпить",
  "хайпим": "хайпить",
  "хайпите": "хайпить",
  "хайпят": "хайпить",
  
  // Формы существительного "краш"
  "краша": "краш",
  "крашу": "краш",
  "крашем": "краш",
  "краше": "краш",
  "крашей": "краш",
  "крашах": "краш",
  "краши": "краш",
  "крашами": "краш",
  
  // Формы "вайб"
  "вайба": "вайб",
  "вайбу": "вайб",
  "вайбом": "вайб",
  "вайбе": "вайб",
  "вайбы": "вайб",
  "вайбов": "вайб",
  "вайбам": "вайб",
  "вайбами": "вайб",
  "вайбах": "вайб",
  
  // Формы "кринж"
  "кринжа": "кринж",
  "кринжу": "кринж",
  "кринжем": "кринж",
  "кринже": "кринж",
  "кринжи": "кринж",
  
  // Формы "батхёрт"
  "батхёрта": "батхёрт",
  "батхёрту": "батхёрт",
  "батхёртом": "батхёрт",
  "батхёрте": "батхёрт",
  "батхёрты": "батхёрт",
  
  // Формы "токсик"
  "токсика": "токсик",
  "токсику": "токсик",
  "токсиком": "токсик",
  "токсике": "токсик",
  "токсики": "токсик",
  "токсиков": "токсик",
  "токсикам": "токсик",
  "токсиками": "токсик",
  "токсиках": "токсик",
  
  // Формы "зумер"
  "зумера": "зумер",
  "зумеру": "зумер",
  "зумером": "зумер",
  "зумере": "зумер",
  "зумеры": "зумер",
  "зумеров": "зумер",
  "зумерам": "зумер",
  "зумерами": "зумер",
  "зумерах": "зумер",
  
  // Формы "бумер"
  "бумера": "бумер",
  "бумеру": "бумер",
  "бумером": "бумер",
  "бумере": "бумер",
  "бумеры": "бумер",
  "бумеров": "бумер",
  "бумерам": "бумер",
  "бумерами": "бумер",
  "бумерах": "бумер",
  
  // Формы "рандом"
  "рандома": "рандом",
  "рандому": "рандом",
  "рандомом": "рандом",
  "рандоме": "рандом",
  "рандомы": "рандом",
  
  // Формы "спойлер"
  "спойлера": "спойлер",
  "спойлеру": "спойлер",
  "спойлером": "спойлер",
  "спойлере": "спойлер",
  "спойлеры": "спойлер",
  "спойлеров": "спойлер",
  
  // Формы "мем"
  "мема": "мем",
  "мему": "мем",
  "мемом": "мем",
  "меме": "мем",
  "мемы": "мем",
  "мемов": "мем",
  "мемам": "мем",
  "мемами": "мем",
  "мемах": "мем",
  
  // Формы "хайп"
  "хайпа": "хайп",
  "хайпу": "хайп",
  "хайпом": "хайп",
  "хайпе": "хайп",
  "хайпы": "хайп",
  
  // Формы "пранк"
  "пранка": "пранк",
  "пранку": "пранк",
  "пранком": "пранк",
  "пранке": "пранк",
  "пранки": "пранк",
  "пранков": "пранк",
  
  // Формы "дроп"
  "дропа": "дроп",
  "дропу": "дроп",
  "дропом": "дроп",
  "дропе": "дроп",
  "дропы": "дроп",
  "дропов": "дроп",
  "дропам": "дроп",
  "дропами": "дроп",
  "дропах": "дроп",
  
  // Глагольные формы "дропать"
  "дропать": "дроп",
  "дропал": "дроп",
  "дропала": "дроп",
  "дропали": "дроп",
  "дропаю": "дроп",
  "дропаешь": "дроп",
  "дропает": "дроп",
  "дропаем": "дроп",
  "дропаете": "дроп",
  "дропают": "дроп",
  "дропнул": "дроп",
  "дропнула": "дроп",
  "дропнули": "дроп",
  "задропать": "дроп",
  "задропал": "дроп",
  "задропала": "дроп",
  
  // Формы "гайд"
  "гайда": "гайд",
  "гайду": "гайд",
  "гайдом": "гайд",
  "гайде": "гайд",
  "гайды": "гайд",
  "гайдов": "гайд",
  
  // Формы "лейм"
  "лейма": "лейм",
  "лейму": "лейм",
  "леймом": "лейм",
  "лейме": "лейм",
  "леймы": "лейм",
  "леймов": "лейм",
  
  // Прилагательные формы "леймовый"
  "леймовый": "лейм",
  "леймового": "лейм",
  "леймовому": "лейм",
  "леймовым": "лейм",
  "леймовом": "лейм",
  "леймовая": "лейм",
  "леймовой": "лейм",
  "леймовую": "лейм",
  "леймовое": "лейм",
  "леймовые": "лейм",
  "леймовых": "лейм",
  "леймовыми": "лейм",
  
  // Формы "лайк"
  "лайка": "лайк",
  "лайку": "лайк",
  "лайком": "лайк",
  "лайке": "лайк",
  "лайки": "лайк",
  "лайков": "лайк",
  "лайкам": "лайк",
  "лайками": "лайк",
  
  // Формы "репост"
  "репоста": "репост",
  "репосту": "репост",
  "репостом": "репост",
  "репосте": "репост",
  "репосты": "репост",
  "репостов": "репост",
  
  // Формы "селфи"
  "селфи": "селфи",
  
  // Формы "контент"
  "контента": "контент",
  "контенту": "контент",
  "контентом": "контент",
  "контенте": "контент",
  
  // Формы "тян"
  "тяна": "тян",
  "тяну": "тян",
  "тяном": "тян",
  "тяне": "тян",
  "тяны": "тян",
  
  // Формы "фейк"
  "фейка": "фейк",
  "фейку": "фейк",
  "фейком": "фейк",
  "фейке": "фейк",
  "фейки": "фейк",
  "фейков": "фейк",
  "фейковый": "фейк",
  "фейковая": "фейк",
  "фейковое": "фейк",
  "фейковые": "фейк",
  
  // Формы "лут"
  "лута": "лут",
  "луту": "лут",
  "лутом": "лут",
  "луте": "лут",
  
  // Формы "скилл"
  "скилла": "скилл",
  "скиллу": "скилл",
  "скиллом": "скилл",
  "скилле": "скилл",
  "скиллы": "скилл",
  "скиллов": "скилл",
  
  // Формы "лаг"
  "лага": "лаг",
  "лагу": "лаг",
  "лагом": "лаг",
  "лаге": "лаг",
  "лаги": "лаг",
  "лагов": "лаг",
  
  // Формы "баг"
  "бага": "баг",
  "багу": "баг",
  "багом": "баг",
  "баге": "баг",
  "баги": "баг",
  "багов": "баг",
  
  // Формы "читер"
  "читера": "читер",
  "читеру": "читер",
  "читером": "читер",
  "читере": "читер",
  "читеры": "читер",
  "читеров": "читер",
  
  // Формы "респ"
  "респа": "респ",
  "респу": "респ",
  "респом": "респ",
  "респе": "респ",
  "респы": "респ",
  
  // Формы "фраг"
  "фрага": "фраг",
  "фрагу": "фраг",
  "фрагом": "фраг",
  "фраге": "фраг",
  "фраги": "фраг",
  "фрагов": "фраг",
  
  // Формы "кемпер"
  "кемпера": "кемпер",
  "кемперу": "кемпер",
  "кемпером": "кемпер",
  "кемпере": "кемпер",
  "кемперы": "кемпер",
  "кемперов": "кемпер",
  
  // Формы "рофл"
  "рофла": "рофл",
  "рофлу": "рофл",
  "рофлом": "рофл",
  "рофле": "рофл",
  
  // Формы "кек"
  "кека": "кек",
  "кеку": "кек",
  "кеком": "кек",
  "кеке": "кек",
  
  // Формы "чел"
  "чела": "чел",
  "челу": "чел",
  "челом": "чел",
  "челе": "чел",
  
  // Формы "братан"
  "братана": "братан",
  "братану": "братан",
  "братаном": "братан",
  "братане": "братан",
  "братаны": "братан",
  "братанов": "братан",
  
  // Формы "кореш"
  "кореша": "кореш",
  "корешу": "кореш",
  "корешом": "кореш",
  "кореше": "кореш",
  "кореши": "кореш",
  "корешей": "кореш",
  
  // Формы "дрип"
  "дрипа": "дрип",
  "дрипу": "дрип",
  "дрипом": "дрип",
  "дрипе": "дрип",
  
  // Формы "челлендж"
  "челленджа": "челлендж",
  "челленджу": "челлендж",
  "челленджем": "челлендж",
  "челлендже": "челлендж",
  "челленджи": "челлендж",
  "челленджей": "челлендж",
  
  // Формы "влог"
  "влога": "влог",
  "влогу": "влог",
  "влогом": "влог",
  "влоге": "влог",
  "влоги": "влог",
  "влогов": "влог",
  
  // Формы "блогер"
  "блогера": "блогер",
  "блогеру": "блогер",
  "блогером": "блогер",
  "блогере": "блогер",
  "блогеры": "блогер",
  "блогеров": "блогер",
  
  // Формы "реакция"
  "реакции": "реакция",
  "реакцию": "реакция",
  "реакцией": "реакция",
  "реакций": "реакция",
  
  // Формы "трек"
  "трека": "трек",
  "треку": "трек",
  "треком": "трек",
  "треке": "трек",
  "треки": "трек",
  "треков": "трек",
  
  // Формы "экс"
  "экса": "экс",
  "эксу": "экс",
  "эксом": "экс",
  "эксе": "экс",
  "эксы": "экс",
  
  // Формы "симпатия"
  "симпатии": "симпатия",
  "симпатию": "симпатия",
  "симпатией": "симпатия",
  "симпатий": "симпатия",
  
  // ===== НОВЫЕ СЛЕНГОВЫЕ СЛОВА =====
  
  // Базовые новые слова (Интернет)
  "абуз": "абуз",
  "афк": "афк",
  "бан": "бан",
  "вайп": "вайп",
  "гг": "гг",
  "жиза": "жиза",
  "лурк": "лурк",
  "найс": "найс",
  "овн": "овн",
  "оффтоп": "оффтоп",
  "рект": "рект",
  "свайп": "свайп",
  "скам": "скам",
  "софт": "софт",
  "спам": "спам",
  "тайминг": "тайминг",
  "фарм": "фарм",
  "чсв": "чсв",
  "акк": "акк",
  "бот": "бот",
  "войс": "войс",
  "деф": "деф",
  "кд": "кд",
  "кулдаун": "кулдаун",
  "смурф": "смурф",
  
  // Новые игровые слова
  "авп": "авп",
  "ачивка": "ачивка",
  "билд": "билд",
  "бафф": "бафф",
  "варик": "варик",
  "ганг": "ганг",
  "данж": "данж",
  "инвайт": "инвайт",
  "камбек": "камбек",
  "мейн": "мейн",
  "нерф": "нерф",
  "пик": "пик",
  "пуш": "пуш",
  "рейд": "рейд",
  "рейнж": "рейнж",
  "саппорт": "саппорт",
  "скоп": "скоп",
  "сплит": "сплит",
  "стак": "стак",
  "тимкилл": "тимкилл",
  "ульт": "ульт",
  "абилка": "абилка",
  "агро": "агро",
  "аое": "аое",
  "баланс": "баланс",
  "дамаг": "дамаг",
  "дебафф": "дебафф",
  "дропрейт": "дропрейт",
  "каст": "каст",
  "крафт": "крафт",
  "мета": "мета",
  "моб": "моб",
  "пет": "пет",
  "рес": "рес",
  "танк": "танк",
  
  // Новые слова эмоций
  "грустить": "грустить",
  "кекать": "кекать",
  "лютить": "лютить",
  "панчить": "панчить",
  "пригорать": "пригорать",
  "сальтить": "сальтить",
  "тильтить": "тильтить",
  "триггерить": "триггерить",
  "фаниться": "фаниться",
  "шокать": "шокать",
  "годить": "годить",
  "душнить": "душнить",
  "залипать": "залипать",
  "кайфовать": "кайфовать",
  "ликовать": "ликовать",
  "ныть": "ныть",
  "радовать": "радовать",
  "стебать": "стебать",
  "страдать": "страдать",
  "угорать": "угорать",
  "фанить": "фанить",
  
  // Новые слова общения
  "базар": "базар",
  "вписка": "вписка",
  "движуха": "движуха",
  "личить": "личить",
  "пабить": "пабить",
  "рандомить": "рандомить",
  "резать": "резать",
  "слать": "слать",
  "стопить": "стопить",
  "фактить": "фактить",
  "вписываться": "вписываться",
  "гнать": "гнать",
  "грузить": "грузить",
  "динамить": "динамить",
  "жестить": "жестить",
  "залетать": "залетать",
  "зашквариться": "зашквариться",
  "катать": "катать",
  "ломиться": "ломиться",
  "мочить": "мочить",
  "палить": "палить",
  "подкатывать": "подкатывать",
  "поджигать": "поджигать",
  "разводить": "разводить",
  "рубиться": "рубиться",
  "тащить": "тащить",
  "фаршить": "фаршить",
  "шарить": "шарить",
  
  // Новые слова стиля жизни
  "буст": "буст",
  "вафлить": "вафлить",
  "жмякать": "жмякать",
  "прожимать": "прожимать",
  "рокировать": "рокировать",
  "трешачить": "трешачить",
  "шеймить": "шеймить",
  "апгрейд": "апгрейд",
  "банчить": "банчить",
  "винить": "винить",
  "глючить": "глючить",
  "додать": "додать",
  "забустить": "забустить",
  "катнуть": "катнуть",
  "лайфхак": "лайфхак",
  "мержить": "мержить",
  "найтить": "найтить",
  "отлакать": "отлакать",
  "пробить": "пробить",
  "скипнуть": "скипнуть",
  
  // Новые слова медиа
  "апвоут": "апвоут",
  "блокнуть": "блокнуть",
  "вирусить": "вирусить",
  "запостить": "запостить",
  "лайкать": "лайкать",
  "монетизировать": "монетизировать",
  "отписаться": "отписаться",
  "подкастить": "подкастить",
  "продвинуть": "продвинуть",
  "скринить": "скринить",
  "тренд": "тренд",
  "хайпануть": "хайпануть",
  "апнуть": "апнуть",
  "вирал": "вирал",
  "клипануть": "клипануть",
  "коллабить": "коллабить",
  "майнить": "майнить",
  "пиарить": "пиарить",
  "ремейк": "ремейк",
  "сабиться": "сабиться",
  "фейлить": "фейлить",
  "чекать": "чекать",
  
  // Новые слова отношений
  "бомбить": "бомбить",
  "галить": "галить",
  "дружить": "дружить",
  "залететь": "залететь",
  "клеить": "клеить",
  "отшить": "отшить",
  "пробежаться": "пробежаться",
  "тянка": "тянка",
  "френдить": "френдить",
  "абьюзить": "абьюзить",
  "влюбляться": "влюбляться",
  "встречаться": "встречаться",
  "изменять": "изменять",
  "кидать": "кидать",
  "любить": "любить",
  "обижать": "обижать",
  "ревновать": "ревновать",
  "сходить": "сходить",
  
  // Формы "абузить"
  "абузить": "абуз",
  "абузил": "абуз",
  "абузила": "абуз",
  "абузило": "абуз",
  "абузили": "абуз",
  "абужу": "абуз",
  "абузишь": "абуз",
  "абузит": "абуз",
  "абузим": "абуз",
  "абузите": "абуз",
  "абузят": "абуз",
  
  // Формы "афкашить"
  "афкашить": "афк",
  "афкашил": "афк",
  "афкашила": "афк",
  "афкашило": "афк",
  "афкашили": "афк",
  "афкашу": "афк",
  "афкашишь": "афк",
  "афкашит": "афк",
  "афкашим": "афк",
  "афкашите": "афк",
  "афкашат": "афк",
  
  // Формы "банить"
  "банить": "бан",
  "банил": "бан",
  "банила": "бан",
  "банило": "бан",
  "банили": "бан",
  "баню": "бан",
  "банишь": "бан",
  "банит": "бан",
  "баним": "бан",
  "баните": "бан",
  "банят": "бан",
  "забанить": "бан",
  "забанил": "бан",
  "забанила": "бан",
  "забанили": "бан",
  
  // Формы "фармить"
  "фармить": "фарм",
  "фармил": "фарм",
  "фармила": "фарм",
  "фармило": "фарм",
  "фармили": "фарм",
  "фармлю": "фарм",
  "фармишь": "фарм",
  "фармит": "фарм",
  "фармим": "фарм",
  "фармите": "фарм",
  "фармят": "фарм",
  
  // Формы "луркать"
  "луркать": "лурк",
  "луркал": "лурк",
  "луркала": "лурк",
  "луркало": "лурк",
  "луркали": "лурк",
  "луркаю": "лурк",
  "луркаешь": "лурк",
  "луркает": "лурк",
  "луркаем": "лурк",
  "луркаете": "лурк",
  "луркают": "лурк",
  
  // Формы "овнить"
  "овнить": "овн",
  "овнил": "овн",
  "овнила": "овн",
  "овнило": "овн",
  "овнили": "овн",
  "овню": "овн",
  "овнишь": "овн",
  "овнит": "овн",
  "овним": "овн",
  "овните": "овн",
  "овнят": "овн",
  "заовнить": "овн",
  
  // Формы "гангать"
  "гангать": "ганг",
  "гангал": "ганг",
  "гангала": "ганг",
  "гангало": "ганг",
  "гангали": "ганг",
  "гангаю": "ганг",
  "гангаешь": "ганг",
  "гангает": "ганг",
  "гангаем": "ганг",
  "гангаете": "ганг",
  "гангают": "ганг",
  
  // Формы "ливать"
  "ливать": "ливать",
  "ливал": "ливать",
  "ливала": "ливать",
  "ливало": "ливать",
  "ливали": "ливать",
  "ливаю": "ливать",
  "ливаешь": "ливать",
  "ливает": "ливать",
  "ливаем": "ливать",
  "ливаете": "ливать",
  "ливают": "ливать",
  "ливнуть": "ливать",
  "ливнул": "ливать",
  "ливнула": "ливать",
  "ливнули": "ливать",
  
  // Формы "пушить"
  "пушить": "пуш",
  "пушил": "пуш",
  "пушила": "пуш",
  "пушило": "пуш",
  "пушили": "пуш",
  "пушу": "пуш",
  "пушишь": "пуш",
  "пушит": "пуш",
  "пушим": "пуш",
  "пушите": "пуш",
  "пушат": "пуш",
  
  // Формы "фидить"
  "фидить": "фидить",
  "фидил": "фидить",
  "фидила": "фидить",
  "фидило": "фидить",
  "фидили": "фидить",
  "фижу": "фидить",
  "фидишь": "фидить",
  "фидит": "фидить",
  "фидим": "фидить",
  "фидите": "фидить",
  "фидят": "фидить",
  
  // Формы "хилить"
  "хилить": "хилить",
  "хилил": "хилить",
  "хилила": "хилить",
  "хилило": "хилить",
  "хилили": "хилить",
  "хилю": "хилить",
  "хилишь": "хилить",
  "хилит": "хилить",
  "хилим": "хилить",
  "хилите": "хилить",
  "хилят": "хилить",
  
  // Формы "вайбить"
  "вайбить": "вайбить",
  "вайбил": "вайбить",
  "вайбила": "вайбить",
  "вайбило": "вайбить",
  "вайбили": "вайбить",
  "вайблю": "вайбить",
  "вайбишь": "вайбить",
  "вайбит": "вайбить",
  "вайбим": "вайбить",
  "вайбите": "вайбить",
  "вайбят": "вайбить",
  
  // Формы "кринжить"
  "кринжить": "кринжить",
  "кринжил": "кринжить",
  "кринжила": "кринжить",
  "кринжило": "кринжить",
  "кринжили": "кринжить",
  "кринжу": "кринжить",
  "кринжишь": "кринжить",
  "кринжит": "кринжить",
  "кринжим": "кринжить",
  "кринжите": "кринжить",
  "кринжат": "кринжить",
  
  // Формы существительных
  "абуза": "абуз",
  "абузу": "абуз",
  "абузом": "абуз",
  "абузе": "абуз",
  
  "бана": "бан",
  "бану": "бан",
  "баном": "бан",
  "бане": "бан",
  "баны": "бан",
  "банов": "бан",
  
  "вайпа": "вайп",
  "вайпу": "вайп",
  "вайпом": "вайп",
  "вайпе": "вайп",
  "вайпы": "вайп",
  
  "варика": "варик",
  "варику": "варик",
  "вариком": "варик",
  "варике": "варик",
  "варики": "варик",
  
  "ганга": "ганг",
  "гангу": "ганг",
  "гангом": "ганг",
  "ганге": "ганг",
  "ганги": "ганг",
  
  "билда": "билд",
  "билду": "билд",
  "билдом": "билд",
  "билде": "билд",
  "билды": "билд",
  "билдов": "билд",
  
  "баффа": "бафф",
  "баффу": "бафф",
  "баффом": "бафф",
  "баффе": "бафф",
  "баффы": "бафф",
  "баффов": "бафф",
  
  "данжа": "данж",
  "данжу": "данж",
  "данжем": "данж",
  "дан��е": "данж",
  "данжи": "данж",
  
  "камбека": "камбек",
  "камбеку": "камбек",
  "камбеком": "камбек",
  "камбеке": "камбек",
  
  "мейна": "мейн",
  "мейну": "мейн",
  "мейном": "мейн",
  "мейне": "мейн",
  "мейны": "мейн",
  
  "пика": "пик",
  "пику": "пик",
  "пиком": "пик",
  "пике": "пик",
  "пики": "пик",
  "пиков": "пик",
  
  "пуша": "пуш",
  "пушу": "пуш",
  "пушем": "пуш",
  "пуше": "пуш",
  "пуши": "пуш",
  
  "рейда": "рейд",
  "рейду": "рейд",
  "рейдом": "рейд",
  "рейде": "рейд",
  "рейды": "рейд",
  "рейдов": "рейд",
  
  "скопа": "скоп",
  "скопу": "скоп",
  "скопом": "скоп",
  "скопе": "скоп",
  
  "ульта": "ульт",
  "ульту": "ульт",
  "ультом": "ульт",
  "ульте": "ульт",
  "ульты": "ульт",
  
  "абилки": "абилка",
  "абилку": "абилка",
  "абилкой": "абилка",
  "абилке": "абилка",
  "абилок": "абилка",
  
  "моба": "моб",
  "мобу": "моб",
  "мобом": "моб",
  "мобе": "моб",
  "мобы": "моб",
  "мобов": "моб",
  
  "танка": "танк",
  "танку": "танк",
  "танком": "танк",
  "танке": "танк",
  "танки": "танк",
  "танков": "танк",
  
  "буста": "буст",
  "бусту": "буст",
  "бустом": "буст",
  "бусте": "буст",
  "бусты": "буст",
  
  "тренда": "тренд",
  "тренду": "тренд",
  "трендом": "тренд",
  "тренде": "тренд",
  "тренды": "тренд",
  "трендов": "тренд",
  
  "тянки": "тянка",
  "тянку": "тянка",
  "тянкой": "тянка",
  "тянке": "тянка",
  "тянок": "тянка",
  
  "базара": "базар",
  "базару": "базар",
  "базаром": "базар",
  "базаре": "базар",
  
  "вписки": "вписка",
  "вписку": "вписка",
  "впиской": "вписка",
  "вписке": "вписка",
  
  "движухи": "движуха",
  "движуху": "движуха",
  "движухой": "движуха",
  "движухе": "движуха",
  
  // Базовые формы русских слов
  "грустно": "грустно",
  "обида": "обида",
  "плохо": "плохо",
  "смешно": "смешно",
  "боян": "боян",
  "симпатия": "симпатия",
  "хвастовство": "хвастовство",
  "атмосфера": "атмосфера",
  "неловко": "неловко",
  "токсичный": "токсичный",
  "молодежь": "молодежь",
  "старик": "старик",
  "легко": "легко",
  "пойдем": "пойдем",
  "случайность": "случайность",
  "раскрытие": "раскрытие",
  "розыгрыш": "розыгрыш",
  "красивый": "красивый",
  "шутка": "шутка",
  "слабак": "слабак",
  "слабый": "слабый",
  "парень": "парень",
  "человек": "человек",
  "мощный": "мощный",
  "сильный": "сильный",
  "новичок": "новичок",
  "профи": "профи",
  "профессионал": "профессионал",
  "выпадение": "выпадение",
  "выпуск": "выпуск",
  "релиз": "релиз",
  "скучный": "скучный",
  "неинтересный": "неинтересный",
  "ажиотаж": "ажиотаж",
  "качество": "качество",
  "инструкция": "инструкция",
  "руководство": "руководство",
  "нравится": "нравится",
  "одобрение": "одобрение",
  "репостить": "репостить",
  "публикация": "публикация",
  "самофото": "самофото",
  "содержание": "содержание",
  "материалы": "материалы",
  "девушка": "девушка",
  "подделка": "подделка",
  "ложь": "ложь",
  "добыча": "добыча",
  "награда": "награда",
  "навык": "навык",
  "умение": "умение",
  "задержка": "задержка",
  "замедление": "замедление",
  "ошибка": "ошибка",
  "мошенник": "мошенник",
  "возрождение": "возрож��ение",
  "убийство": "убийство",
  "отсиживающийся": "отсиживающийся",
  "смех": "смех",
  "хохот": "хохот",
  "насмешка": "насмешка",
  "восхищение": "восхищение",
  "удивление": "удивление",
  "смеюсь": "смеюсь",
  "хохочу": "хохочу",
  "извини": "извини",
  "прости": "прости",
  "нормально": "нормально",
  "хорошо": "хорошо",
  "кстати": "кстати",
  "друг": "друг",
  "приятель": "приятель",
  "стиль": "стиль",
  "лучший": "лучший",
  "высший": "высший",
  "отлично": "отлично",
  "круто": "круто",
  "стыдно": "стыдно",
  "позорно": "позорно",
  "движение": "движение",
  "активность": "активность",
  "тусовка": "тусовка",
  "реально": "реально",
  "правда": "правда",
  "вызов": "вызов",
  "испытание": "испытание",
  "видеоблог": "видеоблог",
  "блоггер": "блоггер",
  "создатель": "создатель",
  "отклик": "отклик",
  "эмоция": "эмоция",
  "песня": "песня",
  "композиция": "композиция",
  "дружба": "дружба",
  "бывший": "бывший",
  "кокетничать": "кокетничать",
  "влечение": "влечение",
  
  // Дополнительные формы глаголов
  "отдыхать": "отдыхать",
  "отдыхал": "отдыхать",
  "отдыхала": "отдыхать",
  "отдыхали": "отдыхать",
  "отдыхаю": "отдыхать",
  "отдыхаешь": "отдыхать",
  "отдыхает": "отдыхать",
  "отдыхаем": "отдыхать",
  "отдыхаете": "отдыхать",
  "отдыхают": "отдыхать",
  
  "смеяться": "смеяться",
  "смеялся": "смеяться",
  "смеялась": "смеяться",
  "смеялись": "смеяться",
  "смеюсь": "смеяться",
  "смеёшься": "смеяться",
  "смеётся": "смеяться",
  "смеёмся": "смеяться",
  "смеётесь": "смеяться",
  "смеются": "смеяться",
  
  "хвастаться": "хвастаться",
  "хвастался": "хвастаться",
  "хвасталась": "хвастаться",
  "хвастались": "хвастаться",
  "хвастаюсь": "хвастаться",
  "хвастаешься": "хвастаться",
  "хвастается": "хвастаться",
  "хвастаемся": "хвастаться",
  "хвастаетесь": "хвастаться",
  "хвастаются": "хвастаться",
  
  "злиться": "злиться",
  "злился": "злиться",
  "злилась": "злиться",
  "злились": "злиться",
  "злюсь": "злиться",
  "злишься": "злиться",
  "злится": "злиться",
  "злимся": "злиться",
  "злитесь": "злиться",
  "злятся": "злиться",
  
  "ненавидеть": "ненавидеть",
  "ненавидел": "ненавидеть",
  "ненавидела": "ненавидеть",
  "ненавидели": "ненавидеть",
  "ненавижу": "ненавидеть",
  "ненавидишь": "ненавидеть",
  "ненавидит": "ненавидеть",
  "ненавидим": "ненавидеть",
  "ненавидите": "ненавидеть",
  "ненавидят": "ненавидеть",
  
  "обвинять": "обвинять",
  "обвинял": "обвинять",
  "обвиняла": "обвинять",
  "обвиняли": "обвинять",
  "обвиняю": "обвинять",
  "обвиняешь": "обвинять",
  "обвиняет": "обвинять",
  "обвиняем": "обвинять",
  "обвиняете": "обвинять",
  "обвиняют": "обвинять",
  "обвини": "обвинять",
  "обвините": "обвинять",
  
  "флиртовать": "флиртовать",
  "флиртовал": "флиртовать",
  "флиртовала": "флиртовать",
  "флиртовали": "флиртовать",
  "флиртую": "флиртовать",
  "флиртуешь": "флиртовать",
  "флиртует": "флиртовать",
  "флиртуем": "флиртовать",
  "флиртуете": "флиртовать",
  "флиртуют": "флиртовать",
  
  "кокетничать": "кокетничать",
  "кокетничал": "кокетничать",
  "кокетничала": "кокетничать",
  "кокетничали": "кокетничать",
  "кокетничаю": "кокетничать",
  "кокетничаешь": "кокетничать",
  "кокетничает": "кокетничать",
  
  // Формы существительных
  "шутка": "шутка",
  "шутки": "шутка",
  "шутке": "шутка",
  "шутку": "шутка",
  "шуткой": "шутка",
  "шутках": "шутка",
  "шуток": "шутка",
  "шуткам": "шутка",
  "шутками": "шутка",
  
  "обида": "обида",
  "обиды": "обида",
  "обиде": "обида",
  "обиду": "обида",
  "обидой": "обида",
  "обидах": "обида",
  "обид": "обида",
  "обидам": "обида",
  "обидами": "обида",
  
  // Формы "парень"
  "парня": "парень",
  "парню": "парень",
  "парнем": "парень",
  "парне": "парень",
  "парни": "парень",
  "парней": "парень",
  "парням": "парень",
  "парнями": "парень",
  "парнях": "парень",
  
  // Формы "слабак"
  "слабака": "слабак",
  "слабаку": "слабак",
  "слабаком": "слабак",
  "слабаке": "слабак",
  "слабаки": "слабак",
  "слабаков": "слабак",
  "слабакам": "слабак",
  "слабаками": "слабак",
  "слабаках": "слабак",
  
  // Формы прилагательного "слабый"
  "слабая": "слабый",
  "слабое": "слабый",
  "слабые": "слабый",
  "слабого": "слабый",
  "слабой": "слабый",
  "слабому": "слабый",
  "слабым": "слабый",
  "слабом": "слабый",
  
  // Формы "новичок"
  "новичка": "новичок",
  "новичку": "новичок",
  "новичком": "новичок",
  "новичке": "новичок",
  "новички": "новичок",
  "новичков": "новичок",
  "новичкам": "новичок",
  
  // Формы "выпадение"
  "выпадения": "выпадение",
  "выпадению": "выпадение",
  "выпадением": "выпадение",
  "выпадении": "выпадение",
  
  // Формы "выпуск"
  "выпуска": "выпуск",
  "выпуску": "выпуск",
  "выпуском": "выпуск",
  "выпуске": "выпуск",
  "выпуски": "выпуск",
  "выпусков": "выпуск",
  
  // Формы "релиз"
  "релиза": "релиз",
  "релизу": "релиз",
  "релизом": "релиз",
  "релизе": "релиз",
  "релизы": "релиз",
  "релизов": "релиз",
  
  // Формы прилагательного "скучный"
  "скучная": "скучный",
  "скучное": "скучный",
  "скучные": "скучный",
  "скучного": "скучный",
  "скучной": "скучный",
  "скучному": "скучный",
  "скучным": "скучный",
  "скучном": "скучный",
  "скучных": "скучный",
  "скучными": "скучный",
  
  // Формы прилагательного "неинтересный"
  "неинтересная": "неинтересный",
  "неинтересное": "неинтересный",
  "неинтересные": "неинтересный",
  "неинтересного": "неинтересный",
  "неинтересной": "неинтересный",
  "неинтересному": "неинтересный",
  "неинтересным": "неинтересный",
  "неинтересном": "неинтересный",
  "неинтересных": "неинтересный",
  "неинтересными": "неинтересный",
  "новичками": "новичок",
  "новичках": "новичок",
  
  // Формы "профи"
  "профессионала": "профессионал",
  "профессионалу": "профессионал",
  "профессионалом": "профессионал",
  "профессионале": "профессионал",
  "профессионалы": "профессионал",
  "профессионалов": "профессионал",
  
  // Формы "девушка"
  "девушки": "девушка",
  "девушке": "девушка",
  "девушку": "девушка",
  "девушкой": "девушка",
  
  // Формы "друг"
  "друга": "друг",
  "другу": "друг",
  "друзья": "друг",
  
  // Формы "стиль"
  "стиля": "стиль",
  "стили": "стиль",
  
  // Формы "песня"
  "песни": "песня",
  "песню": "песня",
  
  // Формы "ажиотаж"
  "ажиотажа": "ажиотаж",
  "ажиотажу": "ажиотаж",
  "ажиотажем": "ажиотаж",
  
  // Формы "качество"
  "качества": "качество",
  "качеству": "качество",
  "качеством": "качество",
  
  // Формы "инструкция"
  "инструкции": "инструкция",
  "инструкцию": "инструкция",
  "инструкцией": "инструкция",
  
  // Формы "руководство"
  "руководства": "руководство",
  "руководству": "руководство",
  "руководством": "руководство",
};

// Russian to Slang reverse mappings
const russianToSlangMap: Record<string, string> = {
  "отдыхать": "чилить",
  "смеяться": "рофлить",
  "хвастаться": "флексить",
  "злиться": "агриться",
  "ненавидеть": "хейтить",
  "обвинять": "леймить",
  "транслировать": "стримить",
  "жертвовать": "донатить",
  "провоцировать": "троллить",
  "сводить": "шипперить",
  "грустно": "скучно",
  "обида": "батхёрт",
  "плохо": "криво",
  "смешно": "лол",
  "боян": "баян",
  "симпатия": "краш",
  "хвастовство": "флекс",
  "атмосфера": "вайб",
  "неловко": "кринж",
  "токсичный": "токсик",
  "молодежь": "зумер",
  "старик": "бумер",
  "легко": "изи",
  "пойдем": "го",
  "случайность": "рандом",
  "раскрытие": "спойлер",
  "розыгрыш": "пранк",
  "красивый": "сасный",
  "шутка": "мем",
  "слабак": "лейм",
  "слабый": "лейм",
  "парень": "чувак",
  "человек": "чувак",
  "мощный": "имба",
  "сильный": "имба",
  "новичок": "нуб",
  "профи": "про",
  "профессионал": "про",
  "выпадение": "дроп",
  "выпуск": "дроп",
  "релиз": "дроп",
  "скучный": "лейм",
  "неинтересный": "лейм",
  "ажиотаж": "хайп",
  "качество": "��однота",
  "инструкция": "гайд",
  "руководство": "гайд",
  "нравится": "лайк",
  "одобрение": "лайк",
  "репостить": "репост",
  "публикация": "репост",
  "самофото": "селфи",
  "содержание": "контент",
  "материалы": "контент",
  "девушка": "тян",
  "подделка": "фейк",
  "ложь": "фейк",
  "добыча": "лут",
  "награда": "лут",
  "навык": "скилл",
  "умение": "скилл",
  "задержка": "лаг",
  "замедление": "лаг",
  "ошибка": "баг",
  "мошенник": "читер",
  "возрождение": "респ",
  "убийство": "фраг",
  "отсиживающийся": "кемпер",
  "смех": "рофл",
  "хохот": "рофл",
  "насмешка": "кек",
  "восхищение": "вау",
  "удивление": "вау",
  "смеюсь": "ору",
  "хохочу": "ору",
  "извини": "сорян",
  "прости": "сорян",
  "человек": "чел",
  "нормально": "норм",
  "хорошо": "норм",
  "кстати": "кста",
  "друг": "братан",
  "приятель": "кореш",
  "стиль": "дрип",
  "лучший": "топ",
  "высший": "топ",
  "отлично": "огонь",
  "круто": "огонь",
  "стыдно": "зашквар",
  "позорно": "зашквар",
  "движение": "движ",
  "активность": "движ",
  "тусовка": "движ",
  "реально": "рил",
  "правда": "рил",
  "вызов": "челлендж",
  "испытание": "челлендж",
  "видеоблог": "влог",
  "блоггер": "блогер",
  "создатель": "блогер",
  "отклик": "реакция",
  "эмоция": "реакция",
  "песня": "трек",
  "композиция": "трек",
  "дружба": "френдзона",
  "бывший": "экс",
  "кокетничать": "флиртовать",
  "влечение": "симпатия",
  
  // ===== НОВЫЕ ОБРАТНЫЕ МАППИНГИ =====
  
  // Интернет
  "злоупотреблять": "абуз",
  "эксплуатировать": "абуз",
  "отсутствовать": "афк",
  "заблокировать": "бан",
  "блокировать": "бан",
  "удаление": "вайп",
  "обнуление": "вайп",
  "жизненно": "жиза",
  "наблюдать": "лурк",
  "отлично": "найс",
  "владеть": "овн",
  "доминировать": "овн",
  "уничтожен": "рект",
  "пролистывание": "свайп",
  "обман": "скам",
  "мошенничество": "скам",
  "программа": "софт",
  "рассылка": "спам",
  "время": "тайминг",
  "собирать": "фарм",
  "важность": "чсв",
  "аккаунт": "акк",
  "скрипт": "бот",
  "голос": "войс",
  "защита": "деф",
  "перезарядка": "кд",
  
  // Игры
  "достижение": "ачивка",
  "сборка": "билд",
  "усиление": "бафф",
  "вариант": "варик",
  "нападение": "ганг",
  "подземелье": "данж",
  "приглашение": "инвайт",
  "возвращение": "камбек",
  "выходить": "ливать",
  "основной": "мейн",
  "ослабление": "нерф",
  "выбор": "пик",
  "атаковать": "пуш",
  "набег": "рейд",
  "дальний": "рейнж",
  "поддержка": "саппорт",
  "прицел": "скоп",
  "разделение": "сплит",
  "накопление": "стак",
  "союзник": "тимкилл",
  "способность": "ульт",
  "кормить": "фидить",
  "лечить": "хилить",
  "агрессия": "агро",
  "площадь": "аое",
  "равновесие": "баланс",
  "урон": "дамаг",
  "шанс": "дропрейт",
  "применение": "каст",
  "создание": "крафт",
  "стратегия": "мета",
  "монстр": "моб",
  "питомец": "пет",
  "воскрешение": "рес",
  "защитник": "танк",
  
  // Эмоции
  "настроение": "вайбить",
  "печалиться": "грустить",
  "задевать": "панчить",
  "оскорблять": "панчить",
  "обижаться": "пригорать",
  "раздражаться": "сальтить",
  "выводить": "тильтить",
  "провоцировать": "триггерить",
  "веселиться": "фаниться",
  "удивлять": "шокать",
  "нравиться": "годить",
  "надоедать": "душнить",
  "зависать": "залипать",
  "наслаждаться": "кайфовать",
  "радоваться": "ликовать",
  "жаловаться": "ныть",
  "веселить": "радовать",
  "подшучивать": "стебать",
  "переживать": "страдать",
  "восхищаться": "фанить",
  
  // Общение
  "разговор": "базар",
  "тусовка": "вписка",
  "активность": "движуха",
  "событие": "движуха",
  "личное": "личить",
  "пить": "пабить",
  "случайно": "рандомить",
  "правда": "резать",
  "отправлять": "слать",
  "останавливать": "стопить",
  "факты": "фактить",
  "приходить": "вписываться",
  "врать": "гнать",
  "напрягать": "грузить",
  "игнорировать": "динамить",
  "круто": "жестить",
  "опозориться": "зашквариться",
  "тусить": "катать",
  "настаивать": "ломиться",
  "побеждать": "мочить",
  "раскрывать": "палить",
  "знакомиться": "подкатывать",
  "соревноваться": "рубиться",
  "выигрывать": "тащить",
  "понимать": "шарить",
  
  // Стиль жизни  
  "восхищение": "вау",
  "ерунда": "вафлить",
  "нажимать": "жмякать",
  "активировать": "прожимать",
  "менять": "рокировать",
  "плохо": "трешачить",
  "стыдить": "шеймить",
  "улучшение": "апгрейд",
  "группа": "банчить",
  "обвинять": "винить",
  "неправильно": "глючить",
  "добавить": "додать",
  "усилить": "забустить",
  "сделать": "катнуть",
  "совет": "лайфхак",
  "объединять": "мержить",
  "ночь": "найтить",
  "получиться": "отлакать",
  "достичь": "пробить",
  "пропустить": "скипнуть",
  
  // Медиа
  "плюс": "апвоут",
  "популярным": "вирусить",
  "опубликовать": "запостить",
  "ставить": "лайкать",
  "зарабатывать": "монетизировать",
  "отписаться": "отписаться",
  "подкаст": "подкастить",
  "раскрутить": "продвинуть",
  "скриншот": "скринить",
  "транслировать": "стримить",
  "тенденция": "тренд",
  "прославиться": "хайпануть",
  "улучшить": "апнуть",
  "вирусное": "вирал",
  "клип": "клипануть",
  "сотрудничать": "коллабить",
  "добывать": "майнить",
  "рекламировать": "пиарить",
  "переделка": "ремейк",
  "подписываться": "сабиться",
  "провалиться": "фейлить",
  "проверять": "чекать",
  
  // Отношения
  "встречаться": "галить",
  "тайно": "галить",
  "забеременеть": "залететь",
  "флиртовать": "клеить",
  "отказать": "отшить",
  "изменить": "пробежаться",
  "девушка": "тянка",
  "дружить": "френдить",
  "использовать": "абьюзить",
  "чувства": "влюбляться",
  "отношения": "встречаться",
  "неверным": "изменять",
  "предавать": "кидать",
  "любовь": "любить",
  "больно": "обижать",
  "ревность": "ревновать",
  "свидание": "сходить",
};

// Dictionary data - Slang to Russian
const dictionary: Record<string, DictionaryEntry> = {
  "чилить": {
    word: "Чилить",
    definition: "Отдыхать, расслабляться, проводить время в спокойной обстановке без стресса и напряжения.",
    shortTranslation: "отдыхать",
    examples: [
      "Сегодня весь день чилил дома, смотрел сериалы",
      "Давай встретимся и просто почилим в парке"
    ],
    category: "Общение"
  },
  "скучно": {
    word: "Скучно",
    definition: "Грустно, печально. В молодежном сленге используется для выражения эмоций грусти или тоски.",
    shortTranslation: "грустно",
    examples: [
      "Мне так скучно без тебя",
      "Погода скучная, настроение тоже"
    ],
    category: "Эмоции"
  },
  "батхёрт": {
    word: "Батхёрт",
    definition: "Обида, досада, болезненная реакция на критику или неудачу. От английского 'butthurt'.",
    shortTranslation: "обида",
    examples: [
      "У него полный батхёрт после вчерашнего спора",
      "Не надо батхёртить по каждому поводу"
    ],
    category: "Эмоции"
  },
  "криво": {
    word: "Криво",
    definition: "Неправильно, плохо, некачественно. Что-то сделано не так, как нужно.",
    shortTranslation: "плохо",
    examples: [
      "Эта программа работает криво",
      "Он криво перевел текст"
    ],
    category: "Общение"
  },
  "лол": {
    word: "Лол",
    definition: "Смешно, потешно. Сокращение от 'LOL' (Laughing Out Loud) - громко смеюсь.",
    shortTranslation: "смешно",
    examples: [
      "Лол, это очень смешно!",
      "Посмотри это видео, там лол"
    ],
    category: "Интернет"
  },
  "баян": {
    word: "Баян",
    definition: "Старая шутка или новость, которую все уже видели. Надоевшая, избитая история.",
    shortTranslation: "боян",
    examples: [
      "Это баян, эту шутку все знают",
      "Опять баян публикуешь?"
    ],
    category: "Интернет"
  },
  "рофлить": {
    word: "Рофлить",
    definition: "Смеяться, шутить, веселиться. От 'ROFL' (Rolling On Floor Laughing).",
    shortTranslation: "смеяться",
    examples: [
      "Мы весь вечер рофлили над мемами",
      "Перестань рофлить, это серьезно"
    ],
    category: "Интернет"
  },
  "краш": {
    word: "Краш",
    definition: "Объект симпатии, влюбленности. Человек, который очень нравится.",
    shortTranslation: "симпатия",
    examples: [
      "Он мой краш уже полгода",
      "Познакомь меня со своим крашем"
    ],
    category: "Отношения"
  },
  "флексить": {
    word: "Флексить",
    definition: "Хвастаться, выпендриваться, показывать свои достижения или вещи.",
    shortTranslation: "хвастаться",
    examples: [
      "Он постоянно флексит новыми кроссовками",
      "Хватит флексить своими оценками"
    ],
    category: "Стиль жизни"
  },
  "флекс": {
    word: "Флекс",
    definition: "Хвастовство, демонстрация своих достижений, вещей или статуса. От английского 'flex'.",
    shortTranslation: "хвастовство",
    examples: [
      "Это просто флекс, ему нечем больше похвастаться",
      "Купил новую машину для флекса"
    ],
    category: "Стиль жизни"
  },
  "вайб": {
    word: "Вайб",
    definition: "Атмосфера, настроение, общее ощущение от места, события или человека. От английского 'vibe'.",
    shortTranslation: "атмосфера",
    examples: [
      "Здесь классный вайб",
      "У этой песни летний вайб"
    ],
    category: "Стиль жизни"
  },
  "кринж": {
    word: "Кринж",
    definition: "Что-то очень неловкое, стыдное, вызывающее дискомфорт. Испанский стыд.",
    shortTranslation: "неловко",
    examples: [
      "Это такой кринж, я не могу на это смотреть",
      "Его поведение было полным кринжем"
    ],
    category: "Эмоции"
  },
  "токсик": {
    word: "Токсик",
    definition: "Токсичный человек, неприятный в общении, распространяющий негатив.",
    shortTranslation: "токсичный",
    examples: [
      "Он полный токсик, лучше с ним не общаться",
      "В этой игре слишком много токсиков"
    ],
    category: "Общение"
  },
  "хейтить": {
    word: "Хейтить",
    definition: "Ненавидеть, критиковать, испытывать неприязнь. От английского 'hate'.",
    shortTranslation: "ненавидеть",
    examples: [
      "Зачем ты его хейтишь?",
      "Хейтеры всегда будут хейтить"
    ],
    category: "Общение"
  },
  "леймить": {
    word: "Леймить",
    definition: "Обвинять кого-то в неудачах, ошибках или поражении. Перекладывать вину на другого игрока или человека. От английского 'blame'.",
    shortTranslation: "обвинять",
    examples: [
      "Не леймь команду, ты сам ошибся",
      "Его постоянно леймят за каждый проигрыш",
      "Хватит меня леймить!"
    ],
    category: "Игры"
  },
  "агриться": {
    word: "Агриться",
    definition: "Злиться, раздражаться, быть в плохом настроении. От английского 'aggressive'.",
    shortTranslation: "злиться",
    examples: [
      "Не агрись, это всего лишь игра",
      "Он всегда агрится по мелочам"
    ],
    category: "Эмоции"
  },
  "зумер": {
    word: "Зумер",
    definition: "Представитель поколения Z, рожденный в конце 1990-х - начале 2010-х.",
    shortTranslation: "молодежь",
    examples: [
      "Типичный зумер, постоянно в телефоне",
      "Зумеры не знают мир без интернета"
    ],
    category: "Общение"
  },
  "бумер": {
    word: "Бумер",
    definition: "Человек старшего поколения, не понимающий современных технологий и трендов.",
    shortTranslation: "старик",
    examples: [
      "Ок, бумер, как скажешь",
      "Не будь бумером, попробуй разобраться"
    ],
    category: "Общение"
  },
  "изи": {
    word: "Изи",
    definition: "Легко, просто, без усилий. От английского 'easy'.",
    shortTranslation: "легко",
    examples: [
      "Это задание изи, справлюсь за минуту",
      "Изи катка, мы выиграли"
    ],
    category: "Игры"
  },
  "го": {
    word: "Го",
    definition: "Пойдем, давай, приглашение к действию. От английского 'go'.",
    shortTranslation: "пойдем",
    examples: [
      "Го играть в футбол",
      "Го в кино сегодня вечером"
    ],
    category: "Общение"
  },
  "рандом": {
    word: "Рандом",
    definition: "Случайность, что-то непредсказуемое. От английского 'random'.",
    shortTranslation: "случайность",
    examples: [
      "Это был полный рандом",
      "Выбери рандомную песню"
    ],
    category: "Игры"
  },
  "спойлер": {
    word: "Спойлер",
    definition: "Информация, раскрывающая сюжет фильма, книги или игры заранее.",
    shortTranslation: "раскрытие",
    examples: [
      "Не говори спойлеры, я еще не смотрел!",
      "Ты мне весь фильм заспойлерил"
    ],
    category: "Медиа"
  },
  "стримить": {
    word: "Стримить",
    definition: "Вести прямую трансляцию в интернете.",
    shortTranslation: "транслировать",
    examples: [
      "Сегодня буду стримить игру",
      "Он каждый день стримит на Твиче"
    ],
    category: "Медиа"
  },
  "донатить": {
    word: "Донатить",
    definition: "Отправлять денежные пожертвования стримеру или создателю контента.",
    shortTranslation: "жертвовать",
    examples: [
      "Я задонатил своему любимому стримеру",
      "Спасибо всем, кто донатит!"
    ],
    category: "Медиа"
  },
  "пранк": {
    word: "Пранк",
    definition: "Розыгрыш, шутка над кем-то. От английского 'prank'.",
    shortTranslation: "розыгрыш",
    examples: [
      "Это был эпичный пранк над другом",
      "Давай заснимем пранк для ютуба"
    ],
    category: "Медиа"
  },
  "троллить": {
    word: "Троллить",
    definition: "Специально провоцировать, дразнить или обманывать кого-то ради смеха.",
    shortTranslation: "провоцировать",
    examples: [
      "Он любит троллить людей в комментариях",
      "Перестань меня троллить!"
    ],
    category: "Общение"
  },
  "шипперить": {
    word: "Шипперить",
    definition: "Желать, чтобы два человека были вместе, поддерживать романтическую ��ару.",
    shortTranslation: "сводить",
    examples: [
      "Я их шипперю с первого сезона",
      "Все шипперят этих персонажей"
    ],
    category: "Отношения"
  },
  "сасный": {
    word: "Сасный",
    definition: "Очень красивый, привлекательный. Искаженное 'сексуальный'.",
    shortTranslation: "красивый",
    examples: [
      "Он такой сасный парень",
      "Сасная фотка, выложи в инсту"
    ],
    category: "Стиль жизни"
  },
  "мем": {
    word: "Мем",
    definition: "Вирусная шутка, изображение или видео, распространяемое в интернете.",
    shortTranslation: "шутка",
    examples: [
      "Этот мем уже все видели",
      "Он постоянно присылает смешные мемы"
    ],
    category: "Интернет"
  },
  "хайп": {
    word: "Хайп",
    definition: "Ажиотаж, повышенное внимание, шумиха вокруг чего-либо.",
    shortTranslation: "ажиотаж",
    examples: [
      "Вокруг нового фильма огромный хайп",
      "Зачем поддерживать этот хайп?"
    ],
    category: "Медиа"
  },
  "годнота": {
    word: "Годнота",
    definition: "Качественный, хороший контент или продукт.",
    shortTranslation: "качество",
    examples: [
      "Этот фильм - настоящая годнота",
      "Где найти такую годноту?"
    ],
    category: "Стиль жизни"
  },
  "гайд": {
    word: "Гайд",
    definition: "Руководство, инструкция, объяснение как что-то сделать.",
    shortTranslation: "инструкция",
    examples: [
      "Посмотри гайд на YouTube",
      "Мне нужен гайд по этой игре"
    ],
    category: "Игры"
  },
  "лейм": {
    word: "Лейм",
    definition: "Слабый, неумелый, скучный. Человек или что-то неинтересное.",
    shortTranslation: "слабак",
    examples: [
      "Чувак, ты лейм",
      "Эта игра лейм, давай в другую"
    ],
    category: "Игры"
  },
  "чувак": {
    word: "Чувак",
    definition: "Парень, человек, друг. Неформальное обращение.",
    shortTranslation: "парень",
    examples: [
      "Чувак, как дела?",
      "Этот чувак классный"
    ],
    category: "Общение"
  },
  "имба": {
    word: "Имба",
    definition: "Несбалансированно сильный, слишком мощный. От 'imbalanced'.",
    shortTranslation: "мощный",
    examples: [
      "Это оружие имба",
      "Имбовый персонаж"
    ],
    category: "Игры"
  },
  "нуб": {
    word: "Нуб",
    definition: "Новичок, неопытный игрок. От 'newbie'.",
    shortTranslation: "новичок",
    examples: [
      "Ты играешь как нуб",
      "Нубы не понимают механик"
    ],
    category: "Игры"
  },
  "про": {
    word: "Про",
    definition: "Профессионал, опытный игрок. От 'professional'.",
    shortTranslation: "профи",
    examples: [
      "Он про-игрок",
      "Играет как про"
    ],
    category: "Игры"
  },
  "дроп": {
    word: "Дроп",
    definition: "Выпадение предмета, награды в игре. Также означает релиз, выпуск новой коллекции или товара.",
    shortTranslation: "выпадение",
    examples: [
      "Мне выпал крутой дроп с босса",
      "Сегодня дроп новой коллекции кроссовок",
      "Дроп легендарки!"
    ],
    category: "Игры"
  },
  "лайк": {
    word: "Лайк",
    definition: "Отметка 'нравится' в социальных сетях. Выражение одобрения.",
    shortTranslation: "нравится",
    examples: [
      "Поставь лайк под моим фото",
      "Набрал тысячу лайков за час"
    ],
    category: "Интернет"
  },
  "репост": {
    word: "Репост",
    definition: "Повторная публикация чужого поста или контента в социальных сетях.",
    shortTranslation: "репостить",
    examples: [
      "Сделай репост этой новости",
      "Спасибо за репост!"
    ],
    category: "Интернет"
  },
  "селфи": {
    word: "Селфи",
    definition: "Фотография самого себя, обычно сделанная на телефон.",
    shortTranslation: "самофото",
    examples: [
      "Давай сделаем селфи вместе",
      "Она постоянно выкладывает селфи"
    ],
    category: "Интернет"
  },
  "контент": {
    word: "Контент",
    definition: "Содержимое, материалы, публикуемые в интернете (видео, фото, тексты).",
    shortTranslation: "содержание",
    examples: [
      "Он создает качественный контент",
      "Нужно больше контента для канала"
    ],
    category: "Интернет"
  },
  "тян": {
    word: "Тян",
    definition: "Девушка. От японского 'тян' - уменьшительно-ласкательный суффикс.",
    shortTranslation: "девушка",
    examples: [
      "Красивая тян прошла мимо",
      "Хочу познакомиться с этой тян"
    ],
    category: "Интернет"
  },
  "фейк": {
    word: "Фейк",
    definition: "Подделка, ложь, неправдивая информация. От английского 'fake'.",
    shortTranslation: "подделка",
    examples: [
      "Это фейк, не верь этой новости",
      "Фейковый аккаунт в соцсетях"
    ],
    category: "Интернет"
  },
  "лут": {
    word: "Лут",
    definition: "Добыча, награда в игре. Предметы, полученные от врагов. От английского 'loot'.",
    shortTranslation: "добыча",
    examples: [
      "Собери весь лут с пола",
      "Какой крутой лут выпал!"
    ],
    category: "Игры"
  },
  "скилл": {
    word: "Скилл",
    definition: "Навык, умение, способность игрока. От английского 'skill'.",
    shortTranslation: "навык",
    examples: [
      "У него хороший скилл в стрельбе",
      "Прокачай этот скилл"
    ],
    category: "Игры"
  },
  "лаг": {
    word: "Лаг",
    definition: "Задержка, замедление в работе игры или программы. От английского 'lag'.",
    shortTranslation: "задержка",
    examples: [
      "У меня лаги, не могу нормально играть",
      "Из-за лагов проиграли"
    ],
    category: "Игры"
  },
  "баг": {
    word: "Баг",
    definition: "Ошибка в программе или игре. От английского 'bug'.",
    shortTranslation: "ошибка",
    examples: [
      "Нашел баг в игре",
      "Это баг или фича?"
    ],
    category: "Игры"
  },
  "читер": {
    word: "Читер",
    definition: "Игрок, использующий читы (запрещенные программы) для получения преимущества.",
    shortTranslation: "мошенник",
    examples: [
      "В команде соперников читер",
      "Его забанили за читы"
    ],
    category: "Игры"
  },
  "респ": {
    word: "Респ",
    definition: "Возрождение персонажа после смерти в игре. От 'respawn'.",
    shortTranslation: "возрождение",
    examples: [
      "Жду респа, скоро вернусь",
      "Время респа - 30 секунд"
    ],
    category: "Игры"
  },
  "фраг": {
    word: "Фраг",
    definition: "Убийство врага в игре. От английского 'frag'.",
    shortTranslation: "убийство",
    examples: [
      "Сделал 20 фрагов за матч",
      "Первый фраг мой!"
    ],
    category: "Игры"
  },
  "кемпер": {
    word: "Кемпер",
    definition: "Игрок, который отсиживается в одном месте, не двигаясь. От 'camping'.",
    shortTranslation: "отсиживающийся",
    examples: [
      "Ненавижу кемперов",
      "Не будь кемпером, двигайся!"
    ],
    category: "Игры"
  },
  "рофл": {
    word: "Рофл",
    definition: "Очень смешно, забавная ситуация. От 'ROFL' - Rolling On Floor Laughing.",
    shortTranslation: "смех",
    examples: [
      "Полный рофл",
      "Это был рофл"
    ],
    category: "Эмоции"
  },
  "кек": {
    word: "Кек",
    definition: "Смех, насмешка. Альтернатива 'лол'.",
    shortTranslation: "смех",
    examples: [
      "Кек, ты серьезно?",
      "Кек, не смешно"
    ],
    category: "Эмоции"
  },
  "вау": {
    word: "Вау",
    definition: "Восхищение, удивление. От английского 'wow'.",
    shortTranslation: "восхищение",
    examples: [
      "Вау, это круто!",
      "Вау, не ожидал"
    ],
    category: "Эмоции"
  },
  "ору": {
    word: "Ору",
    definition: "Очень смешно, хохочу. Сокращение от 'ржу' (ржать - смеяться).",
    shortTranslation: "смеюсь",
    examples: [
      "Ору с этого видео",
      "Просто ору, не могу"
    ],
    category: "Эмоции"
  },
  "сорян": {
    word: "Сорян",
    definition: "Извини, прости. От английского 'sorry'.",
    shortTranslation: "извини",
    examples: [
      "Сорян, опоздал",
      "Сорян за вчера"
    ],
    category: "Эмоции"
  },
  "чел": {
    word: "Чел",
    definition: "Человек, парень. Сокращение от 'человек'.",
    shortTranslation: "человек",
    examples: [
      "Чел, что делаешь?",
      "Это странный чел"
    ],
    category: "Общение"
  },
  "норм": {
    word: "Норм",
    definition: "Нормально, хорошо, приемлемо. Сокращение от 'нормально'.",
    shortTranslation: "нормально",
    examples: [
      "Как дела? - Норм",
      "Это вполне норм"
    ],
    category: "Общение"
  },
  "кста": {
    word: "Кста",
    definition: "Кстати. Сокращенная форма слова 'кстати'.",
    shortTranslation: "кстати",
    examples: [
      "Кста, я забыл тебе сказать",
      "Кста, ты видел новый фильм?"
    ],
    category: "Общение"
  },
  "братан": {
    word: "Братан",
    definition: "Друг, приятель. Дружеское обращение.",
    shortTranslation: "друг",
    examples: [
      "Как дела, братан?",
      "Братан, помоги мне"
    ],
    category: "Общение"
  },
  "кореш": {
    word: "Кореш",
    definition: "Друг, товарищ, близкий приятель.",
    shortTranslation: "друг",
    examples: [
      "Он мой кореш с детства",
      "Кореша не подводят"
    ],
    category: "Общение"
  },
  "дрип": {
    word: "Дрип",
    definition: "Стильная одежда, крутой внешний вид. От английского 'drip'.",
    shortTranslation: "стиль",
    examples: [
      "У него крутой дрип",
      "Сегодня в дрипе вышел"
    ],
    category: "Стиль жизни"
  },
  "топ": {
    word: "Топ",
    definition: "Лучший, высшего качества, отличный.",
    shortTranslation: "лучший",
    examples: [
      "Это топовый трек",
      "Топ контент!"
    ],
    category: "Стиль жизни"
  },
  "огонь": {
    word: "Огонь",
    definition: "Отлично, круто, восхитительно.",
    shortTranslation: "отлично",
    examples: [
      "Твое выступление - огонь!",
      "Песня просто огонь"
    ],
    category: "Стиль жизни"
  },
  "зашквар": {
    word: "Зашквар",
    definition: "Стыдно, позорно, что-то неприемлемое или плохое.",
    shortTranslation: "стыдно",
    examples: [
      "Это полный зашквар",
      "Зашквар так одеваться"
    ],
    category: "Стиль жизни"
  },
  "движ": {
    word: "Движ",
    definition: "Движение, активность, события, тусовка.",
    shortTranslation: "движение",
    examples: [
      "Сегодня будет движ",
      "Какой-то движ намечается"
    ],
    category: "Стиль жизни"
  },
  "рил": {
    word: "Рил",
    definition: "Реально, правда, серьезно. От английского 'real'.",
    shortTranslation: "реально",
    examples: [
      "Это рил произошло?",
      "Рил круто получилось"
    ],
    category: "Стиль жизни"
  },
  "челлендж": {
    word: "Челлендж",
    definition: "Вызов, задание, испытание. Популярный формат в соцсетях.",
    shortTranslation: "вызов",
    examples: [
      "Участвую в новом челлендже",
      "Этот челлендж слишком сложный"
    ],
    category: "Медиа"
  },
  "влог": {
    word: "Влог",
    definition: "Видеоблог, личный видеодневник. От 'video blog'.",
    shortTranslation: "видеоблог",
    examples: [
      "Снял новый влог о путешествии",
      "Его влоги очень интересные"
    ],
    category: "Медиа"
  },
  "блогер": {
    word: "Блогер",
    definition: "Человек, ведущий блог, создающий контент для интернета.",
    shortTranslation: "блоггер",
    examples: [
      "Она известный блогер",
      "Мечтаю стать блогером"
    ],
    category: "Медиа"
  },
  "реакция": {
    word: "Реакция",
    definition: "Видео, где человек реагирует на что-то, показывает свои эмоции.",
    shortTranslation: "отклик",
    examples: [
      "Снял реакцию на этот клип",
      "Посмотри мою реакцию на фильм"
    ],
    category: "Медиа"
  },
  "трек": {
    word: "Трек",
    definition: "Музыкальная композиция, песня.",
    shortTranslation: "песня",
    examples: [
      "Вышел новый трек любимого артиста",
      "Этот трек в топе"
    ],
    category: "Медиа"
  },
  "френдзона": {
    word: "Френдзона",
    definition: "Ситуация, когда один человек хочет отношений, а другой видит только дружбу.",
    shortTranslation: "дружба",
    examples: [
      "Он меня кинул во френдзону",
      "Попал в френдзону"
    ],
    category: "Отношения"
  },
  "экс": {
    word: "Экс",
    definition: "Бывший парень или бывшая девушка. От 'ex'.",
    shortTranslation: "бывший",
    examples: [
      "Встретил своего экса",
      "Экс написал мне сообщение"
    ],
    category: "Отношения"
  },
  "флиртовать": {
    word: "Флиртовать",
    definition: "Кокетничать, проявлять романтический интерес игриво.",
    shortTranslation: "кокетничать",
    examples: [
      "Он со мной флиртует",
      "Они весь вечер флиртовали"
    ],
    category: "Отношения"
  },
  "симпатия": {
    word: "Симпатия",
    definition: "Человек, который нравится, вызывает приятные чувства.",
    shortTranslation: "влечение",
    examples: [
      "У меня есть симпатия в классе",
      "Он моя давняя симпатия"
    ],
    category: "Отношения"
  },
  
  // ===== НОВЫЕ СЛОВА ИНТЕРНЕТ =====
  
  "абуз": {
    word: "Абуз",
    definition: "Злоупотребление, эксплуатация уязвимости или механики в свою пользу. От английского 'abuse'.",
    shortTranslation: "злоупотребление",
    examples: [
      "Это абуз игровой механики",
      "Банят за абуз бага"
    ],
    category: "Интернет"
  },
  "афк": {
    word: "АФК",
    definition: "Отошел от клавиатуры, отсутствует. От английского 'Away From Keyboard'.",
    shortTranslation: "отсутствует",
    examples: [
      "Я афк на 5 минут",
      "Он афкашит уже час"
    ],
    category: "Интернет"
  },
  "бан": {
    word: "Бан",
    definition: "Блокировка доступа к сервису, игре или аккаунту.",
    shortTranslation: "блокировка",
    examples: [
      "Получил бан на неделю",
      "Его забанили за нарушение правил"
    ],
    category: "Интернет"
  },
  "вайп": {
    word: "Вайп",
    definition: "Полное удаление или обнуление прогресса. От английского 'wipe'.",
    shortTranslation: "обнуление",
    examples: [
      "После вайпа все начнут с нуля",
      "Вайп сервера произойдет завтра"
    ],
    category: "Интернет"
  },
  "гг": {
    word: "ГГ",
    definition: "Хорошая игра. От английского 'Good Game'.",
    shortTranslation: "хорошаяигра",
    examples: [
      "ГГ всем, хорошо сыграли",
      "Пишут ГГ в конце матча"
    ],
    category: "Интернет"
  },
  "жиза": {
    word: "Жиза",
    definition: "Жизненно, правда жизни, очень актуально. Сокращение от 'жизненная ситуация'.",
    shortTranslation: "жизненно",
    examples: [
      "Это прям жиза",
      "Такая жиза, со всеми бывает"
    ],
    category: "Интернет"
  },
  "лурк": {
    word: "Лурк",
    definition: "Скрытное наблюдение, чтение без участия в обсуждении. От английского 'lurk'.",
    shortTranslation: "наблюдение",
    examples: [
      "Просто луркаю в чате",
      "Лурк-мод включен"
    ],
    category: "Интернет"
  },
  "найс": {
    word: "Найс",
    definition: "Отлично, хорошо, круто. От английского 'nice'.",
    shortTranslation: "отлично",
    examples: [
      "Найс, продолжай в том же духе",
      "Найс работа!"
    ],
    category: "Интернет"
  },
  "овн": {
    word: "Овн",
    definition: "Владеть, доминировать, побеждать. От английского 'own'.",
    shortTranslation: "доминировать",
    examples: [
      "Он заовнил всех противников",
      "Полный овн в игре"
    ],
    category: "Интернет"
  },
  "оффтоп": {
    word: "Оффтоп",
    definition: "Сообщение не по теме обсуждения. От английского 'off-topic'.",
    shortTranslation: "нептеме",
    examples: [
      "Это оффтоп, давайте вернемся к теме",
      "Оффтоп запрещен в этом чате"
    ],
    category: "Интернет"
  },
  "рект": {
    word: "Рект",
    definition: "Полностью уничтожен, разгромлен. От английского 'rekt' (wrecked).",
    shortTranslation: "уничтожен",
    examples: [
      "Гет рект!",
      "Его просто ректнули"
    ],
    category: "Интернет"
  },
  "свайп": {
    word: "Свайп",
    definition: "Пролистывание экрана пальцем или мышью. От английского 'swipe'.",
    shortTranslation: "пролистывание",
    examples: [
      "Свайп вправо - лайк",
      "Сделай свайп вниз"
    ],
    category: "Интернет"
  },
  "скам": {
    word: "Скам",
    definition: "Обман, мошенничество, развод на деньги. От английского 'scam'.",
    shortTranslation: "мошенничество",
    examples: [
      "Это чистый скам, не ведись",
      "Скам-сайт, не вводи данные"
    ],
    category: "Интернет"
  },
  "софт": {
    word: "Софт",
    definition: "Программное обеспечение, программы. От английского 'software'.",
    shortTranslation: "программа",
    examples: [
      "Установи этот софт",
      "Бесплатный софт для монтажа"
    ],
    category: "Интернет"
  },
  "спам": {
    word: "Спам",
    definition: "Массовая нежелательная рассылка сообщений.",
    shortTranslation: "рассылка",
    examples: [
      "Не спамь в чате",
      "Это просто спам, игнорируй"
    ],
    category: "Интернет"
  },
  "тайминг": {
    word: "Тайминг",
    definition: "Выбор времени, точность момента действия. От английского 'timing'.",
    shortTranslation: "время",
    examples: [
      "Идеальный тайминг!",
      "Все зависит от тайминга"
    ],
    category: "Интернет"
  },
  "фарм": {
    word: "Фарм",
    definition: "Сбор ресурсов, опыта или денег в игре. От английского 'farm'.",
    shortTranslation: "сбор",
    examples: [
      "Иду на фарм",
      "Фармлю голду весь день"
    ],
    category: "Интернет"
  },
  "чсв": {
    word: "ЧСВ",
    definition: "Чувство собственной важности, завышенное самомнение.",
    shortTranslation: "важность",
    examples: [
      "У него ЧСВ зашкаливает",
      "Понизь свое ЧСВ"
    ],
    category: "Интернет"
  },
  "акк": {
    word: "Акк",
    definition: "Аккаунт, учетная запись. Сокращение от 'account'.",
    shortTranslation: "аккаунт",
    examples: [
      "Зарегал новый акк",
      "Мой акк взломали"
    ],
    category: "Интернет"
  },
  "бот": {
    word: "Бот",
    definition: "Автоматический скрипт или очень слабый игрок. От английского 'bot'.",
    shortTranslation: "скрипт",
    examples: [
      "Это точно бот, а не человек",
      "Играешь как бот"
    ],
    category: "Интернет"
  },
  "войс": {
    word: "Войс",
    definition: "Голосовой чат, голосовая связь. От английского 'voice'.",
    shortTranslation: "голос",
    examples: [
      "Зайди в войс",
      "Общаемся в войсе"
    ],
    category: "Интернет"
  },
  
  // ===== НОВЫЕ СЛОВА ИГРЫ =====
  
  "авп": {
    word: "АВП",
    definition: "Снайперская винтовка AWP в Counter-Strike.",
    shortTranslation: "винтовка",
    examples: [
      "Купил авп",
      "С авп играет отлично"
    ],
    category: "Игры"
  },
  "ачивка": {
    word: "Ачивка",
    definition: "Достижение в игре. От английского 'achievement'.",
    shortTranslation: "достижение",
    examples: [
      "Выбил редкую ачивку",
      "Собираю все ачивки"
    ],
    category: "Игры"
  },
  "билд": {
    word: "Билд",
    definition: "Сборка персонажа, комбинация навыков и предметов. От английского 'build'.",
    shortTranslation: "сборка",
    examples: [
      "Попробуй этот билд",
      "Сильный билд для новичков"
    ],
    category: "Игры"
  },
  "бафф": {
    word: "Бафф",
    definition: "Временное усиление персонажа или оружия. От английского 'buff'.",
    shortTranslation: "усиление",
    examples: [
      "Дай бафф на силу",
      "Персонажа забаффили в патче"
    ],
    category: "Игры"
  },
  "варик": {
    word: "Варик",
    definition: "Вариант, возможность. Сокращение от 'вариант'.",
    shortTranslation: "вариант",
    examples: [
      "Какие варики?",
      "Есть варик сходить?"
    ],
    category: "Игры"
  },
  "ганг": {
    word: "Ганг",
    definition: "Групповое нападение на одного игрока. От английского 'gang'.",
    shortTranslation: "нападение",
    examples: [
      "Идем на ганг",
      "Меня гангают постоянно"
    ],
    category: "Игры"
  },
  "данж": {
    word: "Данж",
    definition: "Подземелье, инстанс для группового прохождения. От английского 'dungeon'.",
    shortTranslation: "подземелье",
    examples: [
      "Пойдем в данж",
      "Прошли новый данж"
    ],
    category: "Игры"
  },
  "инвайт": {
    word: "Инвайт",
    definition: "Приглашение в группу, на сервер. От английского 'invite'.",
    shortTranslation: "приглашение",
    examples: [
      "Кинь инвайт в группу",
      "Жду инвайт"
    ],
    category: "Игры"
  },
  "камбек": {
    word: "Камбек",
    definition: "Возвращение в игру после отставания. От английского 'comeback'.",
    shortTranslation: "возвращение",
    examples: [
      "Эпичный камбек!",
      "Сделали камбек с 0:10"
    ],
    category: "Игры"
  },
  "ливать": {
    word: "Ливать",
    definition: "Выходить из игры досрочно. От английского 'leave'.",
    shortTranslation: "выходить",
    examples: [
      "Не ливай из игры",
      "Ливнул посреди матча"
    ],
    category: "Игры"
  },
  "мейн": {
    word: "Мейн",
    definition: "Основной персонаж или роль игрока. От английского 'main'.",
    shortTranslation: "основной",
    examples: [
      "Мой мейн - лучник",
      "Играю на мейне"
    ],
    category: "Игры"
  },
  "нерф": {
    word: "Нерф",
    definition: "Ослабление персонажа или предмета разработчиками. От английского 'nerf'.",
    shortTranslation: "ослабление",
    examples: [
      "Персонажа нерфнули",
      "Ждем нерф этого оружия"
    ],
    category: "Игры"
  },
  "пик": {
    word: "Пик",
    definition: "Выбор персонажа или позиции. От английского 'pick'.",
    shortTranslation: "выбор",
    examples: [
      "Хороший пик!",
      "Фаза пиков началась"
    ],
    category: "Игры"
  },
  "пуш": {
    word: "Пуш",
    definition: "Давление, атака, продвижение вперед. От английского 'push'.",
    shortTranslation: "атака",
    examples: [
      "Идем на пуш",
      "Пушим центральную линию"
    ],
    category: "Игры"
  },
  "рейд": {
    word: "Рейд",
    definition: "Групповой набег на босса или локацию. От английского 'raid'.",
    shortTranslation: "набег",
    examples: [
      "Собираем рейд на босса",
      "Идем в рейд"
    ],
    category: "Игры"
  },
  "рейнж": {
    word: "Рейнж",
    definition: "Дальний бой, дистанция атаки. От английского 'range'.",
    shortTranslation: "дальний",
    examples: [
      "Рейнж персонаж",
      "Атакую с рейнжа"
    ],
    category: "Игры"
  },
  "саппорт": {
    word: "Саппорт",
    definition: "Персонаж поддержки, помощник. От английского 'support'.",
    shortTranslation: "поддержка",
    examples: [
      "Играю на саппорте",
      "Нам нужен саппорт"
    ],
    category: "Игры"
  },
  "скоп": {
    word: "Скоп",
    definition: "Оптический прицел. От английского 'scope'.",
    shortTranslation: "прицел",
    examples: [
      "Стрелял без скопа",
      "Найди скоп для винтовки"
    ],
    category: "Игры"
  },
  "сплит": {
    word: "Сплит",
    definition: "Разделение команды, раздельные действия. От английского 'split'.",
    shortTranslation: "разделение",
    examples: [
      "Идем сплитом",
      "Сплит пуш работает"
    ],
    category: "Игры"
  },
  "стак": {
    word: "Стак",
    definition: "Накопление, складирование предметов или эффектов. От английского 'stack'.",
    shortTranslation: "накопление",
    examples: [
      "Набрал 10 стаков",
      "Стакай этот предмет"
    ],
    category: "Игры"
  },
  "тимкилл": {
    word: "Тимкилл",
    definition: "Убийство союзника. От английского 'teamkill'.",
    shortTranslation: "убийствосоюзника",
    examples: [
      "Случайный тимкилл",
      "Кикнули за тимкиллы"
    ],
    category: "Игры"
  },
  "ульт": {
    word: "Ульт",
    definition: "Ультимативная способность персонажа. От английского 'ultimate'.",
    shortTranslation: "способность",
    examples: [
      "Ульт готов!",
      "Использовал ульт"
    ],
    category: "Игры"
  },
  "фидить": {
    word: "Фидить",
    definition: "Кормить врага убийствами, проигрывать. От английского 'feed'.",
    shortTranslation: "кормить",
    examples: [
      "Перестань фидить!",
      "Он фидит весь матч"
    ],
    category: "Игры"
  },
  "хилить": {
    word: "Хилить",
    definition: "Лечить союзников. От английского 'heal'.",
    shortTranslation: "лечить",
    examples: [
      "Хиль меня!",
      "Хилер не хилит"
    ],
    category: "Игры"
  },
  "абилка": {
    word: "Абилка",
    definition: "Способность персонажа. От английского 'ability'.",
    shortTranslation: "способность",
    examples: [
      "Используй абилку",
      "Крутая абилка"
    ],
    category: "Игры"
  },
  "агро": {
    word: "Агро",
    definition: "Агрессия монстра на игрока. От английского 'aggro'.",
    shortTranslation: "агрессия",
    examples: [
      "Взял агро на себя",
      "Не бери агро"
    ],
    category: "Игры"
  },
  "аое": {
    word: "АОЕ",
    definition: "Урон по площади. От английского 'Area of Effect'.",
    shortTranslation: "площадь",
    examples: [
      "АОЕ урон",
      "Используй АОЕ способность"
    ],
    category: "Игры"
  },
  "баланс": {
    word: "Баланс",
    definition: "Равновесие в игре между персонажами и механиками.",
    shortTranslation: "равновесие",
    examples: [
      "Плохой баланс",
      "Игра хорошо сбалансирована"
    ],
    category: "Игры"
  },
  "дамаг": {
    word: "Дамаг",
    definition: "Урон, повреждения. От английского 'damage'.",
    shortTranslation: "урон",
    examples: [
      "Сколько дамага?",
      "Наносит огромный дамаг"
    ],
    category: "Игры"
  },
  "дебафф": {
    word: "Дебафф",
    definition: "Негативный эффект, ослабление. От английского 'debuff'.",
    shortTranslation: "ослабление",
    examples: [
      "Наложил дебафф",
      "Сними дебаффы"
    ],
    category: "Игры"
  },
  "дропрейт": {
    word: "Дропрейт",
    definition: "Шанс выпадения предмета. От английского 'drop rate'.",
    shortTranslation: "шанс",
    examples: [
      "Низкий дропрейт",
      "Увеличили дропрейт"
    ],
    category: "Игры"
  },
  "каст": {
    word: "Каст",
    definition: "Применение заклинания или способности. От английского 'cast'.",
    shortTranslation: "применение",
    examples: [
      "Прервали каст",
      "Кастую заклинание"
    ],
    category: "Игры"
  },
  "крафт": {
    word: "Крафт",
    definition: "Создание предметов из ресурсов. От английского 'craft'.",
    shortTranslation: "создание",
    examples: [
      "Крафчу оружие",
      "Рецепт крафта"
    ],
    category: "Игры"
  },
  "мета": {
    word: "Мета",
    definition: "Популярная стратегия или тактика. От английского 'meta'.",
    shortTranslation: "стратегия",
    examples: [
      "Это мета сейчас",
      "Играю по мете"
    ],
    category: "Игры"
  },
  "моб": {
    word: "Моб",
    definition: "Монстр, враг в игре. От английского 'mob'.",
    shortTranslation: "монстр",
    examples: [
      "Убей этих мобов",
      "Много мобов вокруг"
    ],
    category: "Игры"
  },
  "пет": {
    word: "Пет",
    definition: "Питомец персонажа. От английского 'pet'.",
    shortTranslation: "питомец",
    examples: [
      "Прокачиваю пета",
      "Сильный пет"
    ],
    category: "Игры"
  },
  "рес": {
    word: "Рес",
    definition: "Воскрешение персонажа. От английского 'resurrection'.",
    shortTranslation: "воскрешение",
    examples: [
      "Дай рес",
      "Реснули команду"
    ],
    category: "Игры"
  },
  "танк": {
    word: "Танк",
    definition: "Защитник с большим запасом здоровья. От английского 'tank'.",
    shortTranslation: "защитник",
    examples: [
      "Играю на танке",
      "Танк держит урон"
    ],
    category: "Игры"
  },
  
  // ===== НОВЫЕ СЛОВА ЭМОЦИИ =====
  
  "вайбить": {
    word: "Вайбить",
    definition: "Создавать настроение, атмосферу.",
    shortTranslation: "создаватьнастроение",
    examples: [
      "Музыка хорошо вайбит",
      "Вайбим под эти треки"
    ],
    category: "Эмоции"
  },
  "грустить": {
    word: "Грустить",
    definition: "Печалиться, быть в подавленном настроении.",
    shortTranslation: "печалиться",
    examples: [
      "Не грусти, все будет хорошо",
      "Грущу без тебя"
    ],
    category: "Эмоции"
  },
  "кекать": {
    word: "Кекать",
    definition: "Смеяться, издеваться.",
    shortTranslation: "смеяться",
    examples: [
      "Все кекают с этого",
      "Кекал весь вечер"
    ],
    category: "Эмоции"
  },
  "лютить": {
    word: "Лютить",
    definition: "Делать что-то экстремальное, злить.",
    shortTranslation: "злить",
    examples: [
      "Что ты лютишь?",
      "Перестань лютить"
    ],
    category: "Эмоции"
  },
  "панчить": {
    word: "Панчить",
    definition: "Задевать, оскорблять острой фразой. От английского 'punch'.",
    shortTranslation: "задевать",
    examples: [
      "Больно панчит словами",
      "Хватит панчить друг друга"
    ],
    category: "Эмоции"
  },
  "пригорать": {
    word: "Пригорать",
    definition: "Злиться, обижаться, раздражаться.",
    shortTranslation: "злиться",
    examples: [
      "Не пригорай",
      "У него пригорает от этого"
    ],
    category: "Эмоции"
  },
  "сальтить": {
    word: "Сальтить",
    definition: "Обижаться, быть раздраженным. От английского 'salty'.",
    shortTranslation: "обижаться",
    examples: [
      "Он сальтит после каждого проигрыша",
      "Не сальти"
    ],
    category: "Эмоции"
  },
  "тильтить": {
    word: "Тильтить",
    definition: "Выводить из себя, доводить до тильта. От английского 'tilt'.",
    shortTranslation: "выводить",
    examples: [
      "Это меня тильтит",
      "Не дай себя затильтить"
    ],
    category: "Эмоции"
  },
  "триггерить": {
    word: "Триггерить",
    definition: "Провоцировать негативную реакцию. От английского 'trigger'.",
    shortTranslation: "провоцировать",
    examples: [
      "Это меня триггерит",
      "Не триггерь его"
    ],
    category: "Эмоции"
  },
  "фаниться": {
    word: "Фаниться",
    definition: "Веселиться, получать удовольствие. От английского 'fun'.",
    shortTranslation: "веселиться",
    examples: [
      "Фанимся на вечеринке",
      "Хорошо пофанились"
    ],
    category: "Эмоции"
  },
  "шокать": {
    word: "Шокать",
    definition: "Удивлять, шокировать.",
    shortTranslation: "удивлять",
    examples: [
      "Это шокает",
      "Шокануло всех"
    ],
    category: "Эмоции"
  },
  "годить": {
    word: "Годить",
    definition: "Нравиться, быть качественным.",
    shortTranslation: "нравиться",
    examples: [
      "Этот фильм не годится",
      "Годный контент"
    ],
    category: "Эмоции"
  },
  "душнить": {
    word: "Душнить",
    definition: "Надоедать, раздражать занудством.",
    shortTranslation: "надоедать",
    examples: [
      "Хватит душнить",
      "Он постоянно душнит"
    ],
    category: "Эмоции"
  },
  "залипать": {
    word: "Залипать",
    definition: "Зависать, застревать в одном месте или занятии.",
    shortTranslation: "зависать",
    examples: [
      "Залип на видосах",
      "Залипаю в телефоне"
    ],
    category: "Эмоции"
  },
  "кайфовать": {
    word: "Кайфовать",
    definition: "Наслаждаться, получать удовольствие.",
    shortTranslation: "наслаждаться",
    examples: [
      "Кайфую от этой музыки",
      "Кайфовали весь вечер"
    ],
    category: "Эмоции"
  },
  "ликовать": {
    word: "Ликовать",
    definition: "Радоваться, торжествовать.",
    shortTranslation: "радоваться",
    examples: [
      "Ликуем после победы",
      "Не стоит так ликовать"
    ],
    category: "Эмоции"
  },
  "ныть": {
    word: "Ныть",
    definition: "Жаловаться, причитать.",
    shortTranslation: "жаловаться",
    examples: [
      "Хватит ныть",
      "Ноет постоянно"
    ],
    category: "Эмоции"
  },
  "угорать": {
    word: "Угорать",
    definition: "Очень сильно смеяться.",
    shortTranslation: "смеяться",
    examples: [
      "Угораем с этого",
      "Угорел до слез"
    ],
    category: "Эмоции"
  },
  "фанить": {
    word: "Фанить",
    definition: "Восхищаться кем-то, быть фанатом.",
    shortTranslation: "восхищаться",
    examples: [
      "Фаню от этого артиста",
      "Она фанит от него"
    ],
    category: "Эмоции"
  },
  
  // ===== НОВЫЕ СЛОВА ОБЩЕНИЕ =====
  
  "базар": {
    word: "Базар",
    definition: "Разговор, обсуждение.",
    shortTranslation: "разговор",
    examples: [
      "Какой базар?",
      "Базар ни о чем"
    ],
    category: "Общение"
  },
  "вписка": {
    word: "Вписка",
    definition: "Тусовка у кого-то дома, встреча компании.",
    shortTranslation: "тусовка",
    examples: [
      "Идем на вписку",
      "Классная вписка была"
    ],
    category: "Общение"
  },
  "движуха": {
    word: "Движуха",
    definition: "Активность, какое-то событие, движение.",
    shortTranslation: "активность",
    examples: [
      "Какая движуха?",
      "Нормальная движуха"
    ],
    category: "Общение"
  },
  "личить": {
    word: "Личить",
    definition: "Писать в личные сообщения.",
    shortTranslation: "писать",
    examples: [
      "Личь мне",
      "Личил вчера"
    ],
    category: "Общение"
  },
  "пабить": {
    word: "Пабить",
    definition: "Пить в пабе, баре.",
    shortTranslation: "пить",
    examples: [
      "Пабимся вечером",
      "Пабили всю ночь"
    ],
    category: "Общение"
  },
  "рандомить": {
    word: "Рандомить",
    definition: "Выбирать случайно.",
    shortTranslation: "выбиратьслучайно",
    examples: [
      "Давай рандомим",
      "Рандомлю персонажа"
    ],
    category: "Общение"
  },
  "резать": {
    word: "Резать",
    definition: "Говорить правду, резать правду-матку.",
    shortTranslation: "говоритьправду",
    examples: [
      "Режь правду",
      "Зарезал так зарезал"
    ],
    category: "Общение"
  },
  "слать": {
    word: "Слать",
    definition: "Отправлять сообщения.",
    shortTranslation: "отправлять",
    examples: [
      "Шли фотки",
      "Слал мемы"
    ],
    category: "Общение"
  },
  "стопить": {
    word: "Стопить",
    definition: "Останавливать, прекращать.",
    shortTranslation: "останавливать",
    examples: [
      "Стопи это",
      "Нужно стопить"
    ],
    category: "Общение"
  },
  "фактить": {
    word: "Фактить",
    definition: "Говорить факты, правду.",
    shortTranslation: "говоритьфакты",
    examples: [
      "Он фактит",
      "Фачу только правду"
    ],
    category: "Общение"
  },
  "палить": {
    word: "Палить",
    definition: "Раскрывать секрет, выдавать информацию.",
    shortTranslation: "раскрывать",
    examples: [
      "Не пали секрет",
      "Спалил местонахождение"
    ],
    category: "Общение"
  },
  "шарить": {
    word: "Шарить",
    definition: "Хорошо разбираться, понимать.",
    shortTranslation: "понимать",
    examples: [
      "Он шарит в этом",
      "Не шарю в физике"
    ],
    category: "Общение"
  },
  
  // ===== НОВЫЕ СЛОВА СТИЛЬ ЖИЗНИ =====
  
  "буст": {
    word: "Буст",
    definition: "Усиление, ускорение, повышение. От английского 'boost'.",
    shortTranslation: "усиление",
    examples: [
      "Дай буст",
      "Получил буст энергии"
    ],
    category: "Стиль жизни"
  },
  "апгрейд": {
    word: "Апгрейд",
    definition: "Улучшение, модернизация. От английского 'upgrade'.",
    shortTranslation: "улучшение",
    examples: [
      "Сделал апгрейд компа",
      "Нужен апгрейд"
    ],
    category: "Стиль жизни"
  },
  "лайфхак": {
    word: "Лайфхак",
    definition: "Полезный совет, хитрость для жизни. От английского 'lifehack'.",
    shortTranslation: "совет",
    examples: [
      "Крутой лайфхак",
      "Лайфхак дня"
    ],
    category: "Стиль жизни"
  },
  "шеймить": {
    word: "Шеймить",
    definition: "Стыдить, публично осуждать. От английского 'shame'.",
    shortTranslation: "стыдить",
    examples: [
      "Хватит шеймить",
      "Его шеймят в интернете"
    ],
    category: "Стиль жизни"
  },
  
  // ===== НОВЫЕ СЛОВА МЕДИА =====
  
  "апвоут": {
    word: "Апвоут",
    definition: "Голос 'за', плюс. От английского 'upvote'.",
    shortTranslation: "плюс",
    examples: [
      "Поставь апвоут",
      "Собрал много апвоутов"
    ],
    category: "Медиа"
  },
  "вирусить": {
    word: "Вирусить",
    definition: "Становиться вирусным, набирать популярность.",
    shortTranslation: "становитьсяпопулярным",
    examples: [
      "Видео вирусит",
      "Пост завирусил"
    ],
    category: "Медиа"
  },
  "запостить": {
    word: "Запостить",
    definition: "Опубликовать пост.",
    shortTranslation: "опубликовать",
    examples: [
      "Запостил фотку",
      "Запости это"
    ],
    category: "Медиа"
  },
  "лайкать": {
    word: "Лайкать",
    definition: "Ставить лайк, отметку 'нравится'.",
    shortTranslation: "ставитьлайк",
    examples: [
      "Лайкай пост",
      "Лайкнул все фотки"
    ],
    category: "Медиа"
  },
  "скринить": {
    word: "Скринить",
    definition: "Делать скриншот экрана.",
    shortTranslation: "делатьскриншот",
    examples: [
      "Заскринь это",
      "Скриню переписку"
    ],
    category: "Медиа"
  },
  "тренд": {
    word: "Тренд",
    definition: "Тенденция, модное направление. От английского 'trend'.",
    shortTranslation: "тенденция",
    examples: [
      "Это сейчас в тренде",
      "Следую трендам"
    ],
    category: "Медиа"
  },
  "вирал": {
    word: "Вирал",
    definition: "Вирусное видео или контент. От английского 'viral'.",
    shortTranslation: "вирусное",
    examples: [
      "Это вирал",
      "Снял вирал"
    ],
    category: "Медиа"
  },
  "коллабить": {
    word: "Коллабить",
    definition: "Сотрудничать, делать коллаборацию. От английского 'collaborate'.",
    shortTranslation: "сотрудничать",
    examples: [
      "Коллаблю с блогером",
      "Давай поколлабим"
    ],
    category: "Медиа"
  },
  "пиарить": {
    word: "Пиарить",
    definition: "Рекламировать, продвигать. От 'PR'.",
    shortTranslation: "рекламировать",
    examples: [
      "Пиарит свой канал",
      "Не пиарься здесь"
    ],
    category: "Медиа"
  },
  "ремейк": {
    word: "Ремейк",
    definition: "Переделка, новая версия. От английского 'remake'.",
    shortTranslation: "переделка",
    examples: [
      "Ремейк старого фильма",
      "Делают ремейк игры"
    ],
    category: "Медиа"
  },
  "чекать": {
    word: "Чекать",
    definition: "Проверять, смотреть. От английского 'check'.",
    shortTranslation: "проверять",
    examples: [
      "Чекни это",
      "Чекаю новости"
    ],
    category: "Медиа"
  },
  
  // ===== НОВЫЕ СЛОВА ОТНОШЕНИЯ =====
  
  "тянка": {
    word: "Тянка",
    definition: "Девушка, девочка. Сленговое обозначение.",
    shortTranslation: "девушка",
    examples: [
      "Симпатичная тянка",
      "Познакомился с тянкой"
    ],
    category: "Отношения"
  },
  "френдить": {
    word: "Френдить",
    definition: "Дружить, добавлять в друзья. От английского 'friend'.",
    shortTranslation: "дружить",
    examples: [
      "Давай френдиться",
      "Френжусь с ним"
    ],
    category: "Отношения"
  },
  "галить": {
    word: "Галить",
    definition: "Встречаться тайно, скрывать отношения.",
    shortTranslation: "встречатьсятайно",
    examples: [
      "Они галят уже месяц",
      "Зачем галить?"
    ],
    category: "Отношения"
  },
  "залететь": {
    word: "Залететь",
    definition: "Забеременеть (сленг).",
    shortTranslation: "забеременеть",
    examples: [
      "Боится залететь",
      "Случайно залетела"
    ],
    category: "Отношения"
  },
  "клеить": {
    word: "Клеить",
    definition: "Флиртовать, пытаться познакомиться.",
    shortTranslation: "флиртовать",
    examples: [
      "Клеит всех подряд",
      "Пытается её склеить"
    ],
    category: "Отношения"
  },
  "отшить": {
    word: "Отшить",
    definition: "Отказать, послать, отвергнуть.",
    shortTranslation: "отказать",
    examples: [
      "Его отшили",
      "Она меня отшила"
    ],
    category: "Отношения"
  },
  "кринжить": {
    word: "Кринжить",
    definition: "Испытывать неловкость, стыд за кого-то.",
    shortTranslation: "стыдиться",
    examples: [
      "Кринжую с его поведения",
      "Это так кринжит"
    ],
    category: "Эмоции"
  },
};

// Popular words list
const popularWords = [
  "чилить", "рофлить", "флексить", "вайб", "кринж", 
  "токсик", "изи", "го", "краш", "хайп", "лайк",
  "селфи", "лут", "скилл", "рофл", "вау", "ору",
  "норм", "братан", "дрип", "топ", "огонь", "челлендж",
  "жиза", "найс", "бан", "фарм", "ганг", "пуш",
  "ульт", "мейн", "вписка", "движуха", "тянка", "буст",
  "тренд", "варик", "абуз", "мета", "шарить", "агро"
];

// Categories
const categories = [
  "Все",
  "Интернет",
  "Игры",
  "Эмоции",
  "Общение",
  "Стиль жизни",
  "Медиа",
  "Отношения"
];

// ========================
// HELPER FUNCTIONS
// ========================

// Морфологический анализ слова
interface WordForm {
  base: string;
  prefix?: string;
  suffix?: string;
  ending?: string;
  isVerb?: boolean;
  isNoun?: boolean;
  isAdjective?: boolean;
}

// Определить форму слова
function analyzeWordForm(word: string): WordForm {
  const lowerWord = word.toLowerCase();
  const result: WordForm = { base: lowerWord };
  
  // Определяем приставки
  const prefixes = ['по', 'за', 'от', 'про', 'при', 'пере', 'на', 'вы', 'раз', 'с'];
  for (const prefix of prefixes) {
    if (lowerWord.startsWith(prefix) && lowerWord.length > prefix.length + 2) {
      result.prefix = prefix;
      const withoutPrefix = lowerWord.substring(prefix.length);
      // Проверяем, существует ли базовая форма без приставки
      if (wordFormsMap[withoutPrefix] || dictionary[withoutPrefix]) {
        result.base = withoutPrefix;
        break;
      }
    }
  }
  
  // Определяем форму глагола
  const verbEndings = {
    infinitive: ['ть', 'ти', 'чь'],
    pastMale: ['л'],
    pastFemale: ['ла'],
    pastNeuter: ['ло'],
    pastPlural: ['ли'],
    present1sg: ['у', 'ю'],
    present2sg: ['ешь', 'ишь', 'ёшь'],
    present3sg: ['ет', 'ит', 'ёт'],
    present1pl: ['ем', 'им', 'ём'],
    present2pl: ['ете', 'ите', 'ёте'],
    present3pl: ['ут', 'ют', 'ат', 'ят'],
    imperative: ['и', 'й'],
    imperativePlural: ['ите', 'йте']
  };
  
  const baseWord = result.prefix ? result.base : lowerWord;
  
  // Проверяем окончания глаголов
  for (const [form, endings] of Object.entries(verbEndings)) {
    for (const ending of endings) {
      if (baseWord.endsWith(ending) && baseWord.length > ending.length + 1) {
        result.ending = ending;
        result.isVerb = true;
        result.suffix = form;
        return result;
      }
    }
  }
  
  // Определяем форму существительного/прилагательного
  const nounEndings = ['а', 'у', 'ом', 'е', 'ы', 'ов', 'ам', 'ами', 'ах', 'ей', 'ёй', 'ий', 'ая', 'ое', 'ые', 'его', 'ого', 'ому', 'ему'];
  for (const ending of nounEndings) {
    if (baseWord.endsWith(ending) && baseWord.length > ending.length + 2) {
      result.ending = ending;
      result.isNoun = true;
      return result;
    }
  }
  
  return result;
}

// Применить морфологию к базовому слову
function applyWordForm(baseWord: string, form: WordForm, originalWord: string): string {
  let result = baseWord.toLowerCase();
  
  // Если исходное слово с большой буквы, сохраняем это
  const shouldCapitalize = originalWord[0] === originalWord[0].toUpperCase();
  
  // Применяем приставку
  if (form.prefix) {
    result = form.prefix + result;
  }
  
  // Для глаголов применяем окончания
  if (form.isVerb && form.ending && form.suffix) {
    // Убираем базовое окончание инфинитива
    if (result.endsWith('ть') || result.endsWith('ти')) {
      result = result.slice(0, -2);
    } else if (result.endsWith('чь')) {
      result = result.slice(0, -2);
    }
    
    // Применяем нужное окончание
    switch (form.suffix) {
      case 'infinitive':
        result = result + 'ть';
        break;
      case 'pastMale':
        result = result + 'л';
        break;
      case 'pastFemale':
        result = result + 'ла';
        break;
      case 'pastNeuter':
        result = result + 'ло';
        break;
      case 'pastPlural':
        result = result + 'ли';
        break;
      case 'present1sg':
        // Удаляем последнюю букву основы для -ю/-у
        if (result.endsWith('и')) {
          result = result.slice(0, -1) + 'ю';
        } else {
          result = result + 'ю';
        }
        break;
      case 'present2sg':
        result = result + 'ешь';
        break;
      case 'present3sg':
        result = result + 'ет';
        break;
      case 'present1pl':
        result = result + 'ем';
        break;
      case 'present2pl':
        result = result + 'ете';
        break;
      case 'present3pl':
        result = result + 'ют';
        break;
      case 'imperative':
        result = result + 'и';
        break;
      case 'imperativePlural':
        result = result + 'ите';
        break;
    }
  }
  
  // Для существительных/прилагательных сохраняем окончание
  if (form.isNoun && form.ending) {
    // Убираем базовое окончание если есть
    const baseEndings = ['', 'а', 'о', 'е', 'я'];
    for (const ending of baseEndings) {
      if (result.endsWith(ending) && ending.length > 0) {
        result = result.slice(0, -ending.length);
        break;
      }
    }
    result = result + form.ending;
  }
  
  // Применяем капитализацию
  if (shouldCapitalize) {
    result = result.charAt(0).toUpperCase() + result.slice(1);
  }
  
  return result;
}

// Normalize word to base form
function normalizeWord(word: string): string {
  const lowerWord = word.toLowerCase().trim();
  return wordFormsMap[lowerWord] || lowerWord;
}

// Translate a single word with morphology
function translateWord(word: string, mode: string): DictionaryEntry | null {
  const normalizedWord = normalizeWord(word);
  console.log(`translateWord - word: "${word}", normalized: "${normalizedWord}", mode: ${mode}`);
  
  if (mode === "slangToRussian") {
    return dictionary[normalizedWord] || null;
  } else {
    // Russian to Slang
    const slangWord = russianToSlangMap[normalizedWord];
    console.log(`russianToSlang - slangWord from map: "${slangWord}"`);
    if (slangWord && dictionary[slangWord]) {
      const entry = dictionary[slangWord];
      return {
        ...entry,
        word: entry.word,
        definition: `Сленговый вариант слова "${word}"`,
        shortTranslation: slangWord,
      };
    }
  }
  
  return null;
}

// Translate word with morphology preservation
function translateWordWithMorphology(word: string, mode: string): string | null {
  const cleanWord = word.toLowerCase().trim();
  const wordForm = analyzeWordForm(cleanWord);
  const normalizedWord = normalizeWord(cleanWord);
  
  if (mode === "slangToRussian") {
    const entry = dictionary[normalizedWord];
    if (entry && entry.shortTranslation) {
      return applyWordForm(entry.shortTranslation, wordForm, word);
    }
  } else {
    // Russian to Slang
    const slangWord = russianToSlangMap[normalizedWord];
    if (slangWord) {
      return applyWordForm(slangWord, wordForm, word);
    }
  }
  
  return null;
}

// Translate sentence with morphology
function translateSentence(text: string, mode: string): any {
  const words = text.split(/\s+/);
  let translatedWords: string[] = [];
  const explanations: Array<{ word: string; explanation: string }> = [];
  const allExamples: string[] = [];
  
  for (const word of words) {
    // Сохраняем пунктуацию
    const punctuation = word.match(/[.,!?;:]+$/)?.[0] || "";
    const cleanWord = word.replace(/[.,!?;:]+$/, "");
    
    if (!cleanWord) {
      translatedWords.push(word);
      continue;
    }
    
    // Пытаемся перевести с сохранением морфологии
    const translatedWord = translateWordWithMorphology(cleanWord, mode);
    
    if (translatedWord) {
      translatedWords.push(translatedWord + punctuation);
      
      // Добавляем объяснение и примеры
      const normalizedWord = normalizeWord(cleanWord);
      if (mode === "slangToRussian") {
        const entry = dictionary[normalizedWord];
        if (entry) {
          explanations.push({
            word: cleanWord,
            explanation: entry.definition
          });
          // Добавляем примеры использования
          if (entry.examples && entry.examples.length > 0) {
            allExamples.push(...entry.examples);
          }
        }
      } else {
        const slangWord = russianToSlangMap[normalizedWord];
        if (slangWord) {
          const entry = dictionary[slangWord];
          if (entry) {
            explanations.push({
              word: slangWord,
              explanation: entry.definition
            });
            // Добавляем примеры использования
            if (entry.examples && entry.examples.length > 0) {
              allExamples.push(...entry.examples);
            }
          }
        }
      }
    } else {
      translatedWords.push(word);
    }
  }
  
  // Удаляем дубликаты примеров
  const uniqueExamples = Array.from(new Set(allExamples));
  
  // Удаляем дубликаты объяснений (по слову)
  const uniqueExplanations = explanations.filter((explanation, index, self) => 
    index === self.findIndex((e) => e.word.toLowerCase() === explanation.word.toLowerCase())
  );
  
  return {
    word: text,
    definition: translatedWords.join(" "),
    shortTranslation: null,
    examples: uniqueExamples.length > 0 ? uniqueExamples : [],
    category: "Перевод",
    explanations: uniqueExplanations.length > 0 ? uniqueExplanations : undefined
  };
}

// ========================
// API ROUTES
// ========================

// Health check endpoint
app.get("/health", (c) => {
  return c.json({ status: "ok", message: "Slang Translator API is running" });
});

// Get dictionary
app.get("/dictionary", (c) => {
  try {
    return c.json({
      success: true,
      dictionary,
      wordFormsMap,
      russianToSlangMap,
      categories,
      popularWords
    });
  } catch (error) {
    console.error("Error fetching dictionary:", error);
    return c.json({ success: false, error: "Failed to fetch dictionary" }, 500);
  }
});

// Translate text
app.post("/translate", async (c) => {
  try {
    const body = await c.req.json();
    const { text, mode } = body;
    
    console.log(`Translation request - text: "${text}", mode: ${mode}`);
    
    if (!text || !mode) {
      return c.json({ 
        success: false, 
        error: "Missing required fields: text and mode" 
      }, 400);
    }
    
    const trimmedText = text.trim();
    const isSingleWord = !trimmedText.includes(" ");
    
    console.log(`Trimmed text: "${trimmedText}", isSingleWord: ${isSingleWord}`);
    
    let result;
    if (isSingleWord) {
      const normalizedWord = normalizeWord(trimmedText);
      console.log(`Normalized word: "${normalizedWord}"`);
      result = translateWord(trimmedText, mode);
      console.log(`Translation result:`, result);
    } else {
      result = translateSentence(trimmedText, mode);
    }
    
    if (!result) {
      console.log(`Translation not found for: "${trimmedText}"`);
      return c.json({
        success: false,
        error: "Translation not found"
      }, 404);
    }
    
    // Update statistics
    try {
      const statsKey = "stats:translations";
      const currentStats = await kv.get(statsKey) || { count: 0, lastUpdate: Date.now() };
      await kv.set(statsKey, { 
        count: currentStats.count + 1, 
        lastUpdate: Date.now() 
      });
    } catch (error) {
      console.error("Error updating stats:", error);
    }
    
    console.log('Returning translation result:', JSON.stringify(result, null, 2));
    return c.json({
      success: true,
      translation: result
    });
  } catch (error) {
    console.error("Error translating text:", error);
    return c.json({ 
      success: false, 
      error: "Translation failed: " + error.message 
    }, 500);
  }
});

// Get popular words
app.get("/popular", (c) => {
  try {
    const popularEntries = popularWords.map(word => ({
      word,
      ...dictionary[word]
    })).filter(Boolean);
    
    return c.json({
      success: true,
      words: popularEntries
    });
  } catch (error) {
    console.error("Error fetching popular words:", error);
    return c.json({ success: false, error: "Failed to fetch popular words" }, 500);
  }
});

// Search/autocomplete
app.get("/search", (c) => {
  try {
    const query = c.req.query("q")?.toLowerCase() || "";
    const mode = c.req.query("mode") || "slangToRussian";
    const limit = parseInt(c.req.query("limit") || "10");
    
    if (!query) {
      return c.json({ success: true, results: [] });
    }
    
    let results: string[] = [];
    
    if (mode === "slangToRussian") {
      // Search in slang words
      results = Object.keys(dictionary)
        .filter(word => word.toLowerCase().includes(query))
        .slice(0, limit);
    } else {
      // Search in Russian words
      results = Object.keys(russianToSlangMap)
        .filter(word => word.toLowerCase().includes(query))
        .slice(0, limit);
    }
    
    return c.json({
      success: true,
      results
    });
  } catch (error) {
    console.error("Error searching:", error);
    return c.json({ success: false, error: "Search failed" }, 500);
  }
});

// Get/Set favorites
app.get("/favorites/:userId", async (c) => {
  try {
    const userId = c.req.param("userId");
    const favorites = await kv.get(`favorites:${userId}`) || [];
    
    return c.json({
      success: true,
      favorites
    });
  } catch (error) {
    console.error("Error fetching favorites:", error);
    return c.json({ success: false, error: "Failed to fetch favorites" }, 500);
  }
});

app.post("/favorites/:userId", async (c) => {
  try {
    const userId = c.req.param("userId");
    const body = await c.req.json();
    const { favorites } = body;
    
    await kv.set(`favorites:${userId}`, favorites);
    
    return c.json({
      success: true,
      message: "Favorites updated"
    });
  } catch (error) {
    console.error("Error updating favorites:", error);
    return c.json({ success: false, error: "Failed to update favorites" }, 500);
  }
});

// Get/Set history
app.get("/history/:userId", async (c) => {
  try {
    const userId = c.req.param("userId");
    const history = await kv.get(`history:${userId}`) || [];
    
    return c.json({
      success: true,
      history
    });
  } catch (error) {
    console.error("Error fetching history:", error);
    return c.json({ success: false, error: "Failed to fetch history" }, 500);
  }
});

app.post("/history/:userId", async (c) => {
  try {
    const userId = c.req.param("userId");
    const body = await c.req.json();
    const { history } = body;
    
    await kv.set(`history:${userId}`, history);
    
    return c.json({
      success: true,
      message: "History updated"
    });
  } catch (error) {
    console.error("Error updating history:", error);
    return c.json({ success: false, error: "Failed to update history" }, 500);
  }
});

app.delete("/history/:userId", async (c) => {
  try {
    const userId = c.req.param("userId");
    await kv.del(`history:${userId}`);
    
    return c.json({
      success: true,
      message: "History cleared"
    });
  } catch (error) {
    console.error("Error clearing history:", error);
    return c.json({ success: false, error: "Failed to clear history" }, 500);
  }
});

// Get stats
app.get("/stats", async (c) => {
  try {
    const stats = await kv.get("stats:translations") || { count: 0, lastUpdate: Date.now() };
    
    return c.json({
      success: true,
      stats: {
        totalTranslations: stats.count,
        totalWords: Object.keys(dictionary).length,
        categories: categories.length - 1, // Exclude "Все"
        lastUpdate: stats.lastUpdate
      }
    });
  } catch (error) {
    console.error("Error fetching stats:", error);
    return c.json({ success: false, error: "Failed to fetch stats" }, 500);
  }
});

// Get words by category
app.get("/words/:category", (c) => {
  try {
    const category = c.req.param("category");
    
    // Filter words by category (or return all if "Все")
    let words: DictionaryEntry[];
    
    if (category === "Все" || category === "all") {
      words = Object.values(dictionary);
    } else {
      words = Object.values(dictionary).filter(entry => entry.category === category);
    }
    
    // Sort words alphabetically by word property
    words.sort((a, b) => a.word.localeCompare(b.word, 'ru'));
    
    return c.json({
      success: true,
      category,
      count: words.length,
      words
    });
  } catch (error) {
    console.error("Error fetching words by category:", error);
    return c.json({ success: false, error: "Failed to fetch words" }, 500);
  }
});

// Catch-all 404
app.all("*", (c) => {
  return c.json({ error: "Not Found" }, 404);
});

Deno.serve(app.fetch);
